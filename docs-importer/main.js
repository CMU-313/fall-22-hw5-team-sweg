...
// Welcome message
console.log(
  "Teedy Importer 1.9, https://teedy.io" +
    "\n\n" +
    ...
    .prompt([
      {
        ...
        name: "baseUrl",
        message:
          "What is the base URL of your Teedy? (eg. https://teedy.mycompany.com)",
    ...
    .prompt([
      {
        ...
        name: "fileFilter",
        message:
          "What pattern do you want to use to match files? (eg. *.+(pdf|txt|jpg))",
...
const askLang = () => {
  ...
  request.get(
    ...
    function (error, response, body) {
      ...
      const defaultLang = prefs.importer.lang
        ? prefs.importer.lang
        : JSON.parse(body).default_language;
    ...
    .prompt([
      {
        ...
        name: "copyFolder",
        message:
          "Enter a path to copy files before they are deleted or leave empty to disable. The path must end with a '/' on MacOS and Linux or with a '\\' on Windows. Entering 'undefined' will disable this again after setting the folder.",
        default: prefs.importer.copyFolder,
      },
    ])
    .then((answers) => {
      // Save path
      prefs.importer.copyFolder =
        answers.copyFolder == "undefined" ? "" : answers.copyFolder;
    ...
    .prompt([
      {
        ...
        name: "daemon",
        message:
          "Do you want to run the importer in daemon mode (it will poll the input directory for new files, import and delete them)?",
...
const importFiles = (remove, filesImported) => {
  recursive(prefs.importer.path, function (error, files) {
    files = files.filter(
      minimatch.filter(prefs.importer.fileFilter || "*", { matchBase: true })
    );
    if (files.length === 0) {
      filesImported();
      return;
...
const importFile = (file, remove, resolve) => {
  ...
  let taglist = filename.match(/#[^\s:#]+/gm);
  ...
  request.get(
    ...
    function (error, response, body) {
      ...
      for (let j of taglist) {
        ...
        if (
          j.includes(".") &&
          !tagsarray.hasOwnProperty(j) &&
          !foundtags.includes(tagsarray[j])
        ) {
      ...
      if (
        prefs.importer.tag !== "" &&
        !foundtags.includes(prefs.importer.tag)
      ) {
      ...
      if (prefs.importer.addtags) {
        data = {
          title: prefs.importer.addtags
            ? filename
            : file.replace(/^.*[\\\/]/, "").substring(0, 100),
            ...
            function (error, response) {
              ...
              if (remove) {
                if (prefs.importer.copyFolder) {
                  fs.copyFileSync(
                    file,
                    prefs.importer.copyFolder + file.replace(/^.*[\\\/]/, "")
                  );
                  fs.unlinkSync(file);
                } else {
                  fs.unlinkSync(file);
...
if (argv.hasOwnProperty("d")) {
  console.log(
    "Starting in quiet mode with the following configuration:\n" +
      "Base URL: " +
      prefs.importer.baseUrl +
      "\n" +
      "Username: " +
      prefs.importer.username +
      "\n" +
      "Password: ***********\n" +
      "Tag: " +
      prefs.importer.tag +
      "\n" +
      "Add tags given #: " +
      prefs.importer.addtags +
      "\n" +
      "Language: " +
      prefs.importer.lang +
      "\n" +
      "Daemon mode: " +
      prefs.importer.daemon +
      "\n" +
      "Copy folder: " +
      prefs.importer.copyFolder +
      "\n" +
      "File filter: " +
      prefs.importer.fileFilter
  );
  start();
} else {
  askBaseUrl();
