...
angular.module("yaru22.angular-timeago").factory("timeAgo", [
  ...
  function ($filter, timeAgoSettings) {
    ...
    service.inWords = function (distanceMillis, fromTime, format, timezone) {
      const fullDateAfterSeconds = parseInt(
        timeAgoSettings.fullDateAfterSeconds,
        10
      );
      ...
      const overrideLang = timeAgoSettings.overrideLang;
      ...
      const breakpoints = timeAgoSettings.breakpoints;
      const words =
        (seconds < breakpoints.secondsToMinute &&
          substitute($l.seconds, seconds)) ||
        (seconds < breakpoints.secondsToMinutes && substitute($l.minute, 1)) ||
        (minutes < breakpoints.minutesToHour &&
