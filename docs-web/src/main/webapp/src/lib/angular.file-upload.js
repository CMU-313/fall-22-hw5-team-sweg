  ...
  function ($http, $q, $timeout) {
    ...
    function sendHttp(config) {
      ...
      if (!resumeSupported) {
        ...
        $http.get(config.resumeSizeUrl).then(
          function (resp) {
            if (config.resumeSizeResponseReader) {
              config._start = config.resumeSizeResponseReader(resp.data);
            } else {
              config._start = parseInt(
                (resp.data.size == null ? resp.data : resp.data.size).toString()
              );
            }
      ...
      function toResumeFile(file, formData) {
        ...
        if (config._start != null && resumeSupported) {
          ...
          if (config._chunkSize) {
            ...
            formData.append(
              "_chunkNumber",
              Math.floor(config._start / config._chunkSize)
            );
        ...
        if (val !== undefined) {
          ...
          if (angular.isString(val)) {
            ...
            formData.append(
              key,
              file,
              file.ngfName || encodeURIComponent(file.name)
            );
          } else {
            if (angular.isObject(val)) {
              if (val.$$ngfCircularDetection)
                throw (
                  "ngFileUpload: Circular reference in config.data. Make sure specified data for Upload.upload() has no circular reference: " +
                  key
                );
              ...
              try {
                for (const k in val) {
                  if (val.hasOwnProperty(k) && k !== "$$ngfCircularDetection") {
                    let objectKey =
                      config.objectKey == null ? "[i]" : config.objectKey;
                    if (val.length && parseInt(k) > -1) {
                      objectKey =
                        config.arrayKey == null ? objectKey : config.arrayKey;
                    }
                    addFieldToFormData(
                      formData,
                      val[k],
                      key + objectKey.replace(/[ik]/g, k)
                    );
                  }
                }
              } finally {
    ...
    this.urlToBlob = function (url) {
      const defer = $q.defer();
      $http({ url: url, method: "get", responseType: "arraybuffer" }).then(
        function (resp) {
          const arrayBufferView = new Uint8Array(resp.data);
          const type = resp.headers("content-type") || "image/WebP";
          const blob = new window.Blob([arrayBufferView], { type: type });
  ...
  function ($parse, $timeout, $compile, $q, UploadExif) {
    ...
    function applyExifRotations(files, attr, scope) {
      ...
      angular.forEach(files, function (f, i) {
        if (
          f.type.indexOf("image/jpeg") === 0 &&
          upload.attrGetter("ngfFixOrientation", attr, scope, { $file: f })
        ) {
          promises.push(
            upload
              .happyPromise(upload.applyExifRotation(f), f)
              .then(function (fixedFile) {
    ...
    function resizeFile(files, attr, scope, ngModel) {
      ...
      if (resizeVal instanceof Function) {
        ...
        return resizeVal(files).then(
          function (p) {
            resizeWithParams(p, files, attr, scope, ngModel).then(
              function (r) {
                defer.resolve(r);
              },
              function (e) {
        ...
        if (f.type.indexOf("image") === 0) {
          ...
          promise.then(
            ...
            function (e) {
              ...
              f.$errorParam =
                (e ? (e.message ? e.message : e) + ": " : "") + (f && f.name);
    ...
    upload.updateModel = function (
      ...
      function update(files, invalidFiles, newFiles, dupFiles, isSingleModel) {
        ...
        const invalidFile =
          invalidFiles && invalidFiles.length ? invalidFiles[0] : null;
        ...
        if (invalidModel) {
          $timeout(function () {
            $parse(invalidModel).assign(
              scope,
              isSingleModel ? invalidFile : invalidFiles
            );
          });
        }
        $timeout(function () {
      ...
      function removeDuplicates() {
        function equals(f1, f2) {
          return (
            f1.name === f2.name &&
            (f1.$ngfOrigSize || f1.size) === (f2.$ngfOrigSize || f2.size) &&
            f1.type === f2.type
          );
        }
      ...
      function resizeAndUpdate() {
        function updateModel() {
          $timeout(
            function () {
              update(
                keep ? prevValidFiles.concat(valids) : valids,
      ...
      if (
        keep === "distinct" ||
        upload.attrGetter("ngfKeepDistinct", attr, scope) === true
      ) {
      ...
      var isSingleModel =
        !keep &&
        !upload.attrGetter("ngfMultiple", attr, scope) &&
        !upload.attrGetter("multiple", attr);
      ...
      upload
        ...
        .then(function (validationResult) {
          if (noDelay) {
            ...
            if (
              upload.attrGetter("ngfFixOrientation", attr, scope) &&
              upload.isExifSupported()
            ) {
  ...
  function ($parse, $timeout, $compile, Upload) {
    ...
    function isDelayedClickSupported(ua) {
      ...
      if (m && m.length > 2) {
        ...
        return (
          parseInt(m[1]) < 4 || (parseInt(m[1]) === v && parseInt(m[2]) < v)
        );
      }
    ...
    function linkFileSelect(
      ...
      function detectSwipe(evt) {
        const touches =
          evt.changedTouches ||
          (evt.originalEvent && evt.originalEvent.changedTouches);
        if (touches) {
          if (evt.type === "touchstart") {
            ...
            if (evt.type === "touchend") {
              ...
              if (
                Math.abs(currentX - initialTouchStartX) > 20 ||
...
(function () {
  ngFileUpload.service("UploadDataUrl", [
    ...
    function (UploadBase, $timeout, $q) {
      ...
      upload.dataUrl = function (file, disallowObjectUrl) {
        ...
        if (
          (disallowObjectUrl && file.$ngfDataUrl != null) ||
          (!disallowObjectUrl && file.$ngfBlobUrl != null)
        ) {
          return upload.emptyPromise(
            disallowObjectUrl ? file.$ngfDataUrl : file.$ngfBlobUrl,
            file
          );
        }
        let p = disallowObjectUrl
            ...
            if (URL && URL.createObjectURL && !disallowObjectUrl) {
              ...
              $timeout(function () {
                ...
                if (url) {
                  ...
                  while (
                    (upload.blobUrlsTotalSize > maxMemory ||
                      upload.blobUrls.length > maxLength) &&
                    upload.blobUrls.length > 1
                  ) {
        ...
        p["finally"](function () {
          delete file[
            disallowObjectUrl ? "$$ngfDataUrlPromise" : "$$ngfBlobUrlPromise"
          ];
        });
        return p;
      };
  ...
  function linkFileDirective(
    ...
    function constructDataUrl(file) {
      const disallowObjectUrl = Upload.attrGetter(
        "ngfNoObjectUrl",
        attr,
        scope
      );
    ...
    $timeout(function () {
      const unwatch = scope.$watch(attr[directiveName], function (file) {
        ...
        if (
          file &&
          file.type &&
          file.type.search(getTagType(elem[0])) === 0 &&
...
ngFileUpload.service("UploadValidate", [
  ...
  function (UploadDataUrl, $q, $timeout) {
    ...
    upload.validatePattern = function (file, val) {
      ...
      const pattern = globStringToRegex(val);
      let valid = true;
      if (pattern.regexp && pattern.regexp.length) {
        const regexp = new RegExp(pattern.regexp, "i");
        valid =
          (file.type != null && regexp.test(file.type)) ||
      ...
      while (len--) {
        ...
        valid =
          valid &&
          (file.type == null || exclude.test(file.type)) &&
    ...
    upload.ratioToFloat = function (val) {
      let r = val.toString();
      const xIndex = r.search(/[x:]/i);
      if (xIndex > -1) {
        r =
          parseFloat(r.substring(0, xIndex)) /
          parseFloat(r.substring(xIndex + 1));
      } else {
        r = parseFloat(r);
      }
      return r;
    ...
    upload.getValidationAttr = function (
      attr,
      scope,
      name,
      validationName,
    ...
    upload.validate = function (files, prevLength, ngModel, attr, scope) {
      ...
      const ignoredErrors = (
        upload.attrGetter("ngfIgnoreInvalid", attr, scope) || ""
      ).split(" ");
      let runAllValidation = upload.attrGetter(
        "ngfRunAllValidations",
        attr,
        scope
      );
      ...
      function validateSync(name, validationName, fn) {
        if (files) {
          ...
          while (i--) {
            const file = files[i];
            if (file) {
              const val = upload.getValidationAttr(
                attr,
                scope,
                name,
                validationName,
      ...
      function validateAsync(name, validationName, type, asyncFn, fn) {
        ...
        if (files) {
          ...
          angular.forEach(files, function (file) {
            ...
            if (
              name === "dimensions" &&
              upload.attrGetter("ngfDimensions", attr) != null
            ) {
              ...
              name === "duration" &&
              upload.attrGetter("ngfDuration", attr) != null
            ) {
              upload.mediaDuration(file).then(
                function (d) {
                  resolveResult(
                    defer,
                    file,
      ...
      promises.push(
        validateAsync(
          ...
          function (d, val) {
            ...
            for (let i = 0; i < split.length; i++) {
              if (
                Math.abs(d.width / d.height - upload.ratioToFloat(split[i])) <
                0.01
              ) {
                valid = true;
              }
    ...
    upload.mediaDuration = function (file) {
      ...
      $timeout(function () {
        ...
        upload.dataUrl(file).then(
          function (dataUrl) {
            const el = angular
              .element(file.type.indexOf("audio") === 0 ? "<audio>" : "<video>")
              .attr("src", dataUrl)
              .css("visibility", "none")
              .css("position", "fixed");
...
ngFileUpload.service("UploadResize", [
  ...
  function (UploadValidate, $q) {
    ...
    const resize = function (
      ...
      imageElement.onload = function () {
        ...
        try {
          ...
          deferred.resolve(
            canvasElement.toDataURL(type || "image/WebP", quality || 0.934)
          );
        } catch (e) {
          deferred.reject(e);
        }
    ...
    upload.isResizeSupported = function () {
      ...
      return (
        window.atob && elem.getContext && elem.getContext("2d") && window.Blob
      );
    };
        ...
        function (url) {
          resize(
            ...
            function (dataUrl) {
              ...
              try {
                const blob = upload.dataUrltoBlob(
                  dataUrl,
                  file.name,
                  file.size
                );
...
(function () {
  ...
  function linkDrop(
    ...
    function extractFilesFromHtml(updateOn, html) {
      if (
        !upload.shouldUpdateOn(updateOn, attr, scope) ||
        typeof html !== "string"
      )
    ...
    function calculateDragOverClass(scope, attr, evt, callback) {
      const obj = attrGetter("ngfDragOverClass", scope, { $event: evt });
      let dClass = "dragover";
      if (angular.isString(obj)) {
        ...
        if (obj.accept || obj.reject) {
          ...
          if (items == null || !items.length) {
            ...
            const pattern =
              obj.pattern || attrGetter("ngfPattern", scope, { $event: evt });
...
ngFileUpload.service("UploadExif", [
  ...
  function (UploadResize, $q) {
    ...
    upload.applyExifRotation = function (file) {
      ...
      const deferred = $q.defer();
      upload.readOrientation(file).then(
        function (result) {
          ...
          upload.dataUrl(file, true).then(
            function (url) {
              ...
              img.onload = function () {
                try {
                  canvas.width =
                    result.orientation > 4 ? img.height : img.width;
                  canvas.height =
                    result.orientation > 4 ? img.width : img.height;
    ...
    upload.restoreExif = function (orig, resized) {
      ...
      ExifRestorer.restore = function (origFileBase64, resizedFileBase64) {
        if (origFileBase64.match("data:image/jpeg;base64,")) {
          origFileBase64 = origFileBase64.replace(
      ...
      ExifRestorer.insertExif = function (resizedFileBase64, exifArray) {
        const imageData = resizedFileBase64.replace(
          "data:image/jpeg;base64,",
          ""
        );
      ...
      ExifRestorer.slice2Segments = function (rawImageArray) {
        ...
        while (1) {
          ...
          if (
            ...
            const length =
              rawImageArray[head + 2] * 256 + rawImageArray[head + 3];
