 ...
 */
  ...
  function adjustCSS( elem, prop, valueParts, tween ) {
    ...
    let initialInUnit = ( jQuery.cssNumber[prop] || unit !== "px" && +initial ) &&
  ...
  function buildFragment( elems, context, scripts, selection, ignored ) {
    let elem; let tmp; let tag; let wrap; let contains; let j;
 ...
 */
  ...
  function restoreScript( elem ) {
    const match = rscriptTypeMasked.exec( elem.type );
  ...
  jQuery.extend( {
    ...
    clone: function( elem, dataAndEvents, deepDataAndEvents ) {
      ...
      const inPage = jQuery.contains( elem.ownerDocument, elem );
  ...
  ( function() {
    ...
    let pixelPositionVal; let boxSizingReliableVal; let pixelMarginRightVal; let reliableMarginLeftVal;
  ...
  jQuery.fn.extend( {
    val: function( value ) {
      let hooks; let ret; let isFunction;
      const elem = this[0];
 ...
 */
  ...
  jQuery.fn.extend( {
    offset: function( options ) {
      ...
      let doc; let docElem; let rect; let win;
      const elem = this[0];
