...
angular.module("ui.bootstrap.alert", [])
    ...
    .controller("UibAlertController", ["$scope", "$element", "$attrs", "$interpolate", "$timeout", function($scope, $element, $attrs, $interpolate, $timeout) {
      ...
      const dismissOnTimeout = angular.isDefined($attrs.dismissOnTimeout) ?
    ...
    .directive("uibBtnRadio", ["$parse", function($parse) {
      return {
        ...
        link: function(scope, element, attrs, ctrls) {
          const buttonsCtrl = ctrls[0]; const ngModelCtrl = ctrls[1];
          const uncheckableExpr = $parse(attrs.uibUncheckable);
...
angular.module("ui.bootstrap.carousel", [])
    ...
    .controller("UibCarouselController", ["$scope", "$element", "$interval", "$timeout", "$animate", function($scope, $element, $interval, $timeout, $animate) {
      ...
      let currentInterval; let isPlaying;
        ...
        return {
          beforeAddClass: function(element, className, done) {
            if (className === "active") {
              ...
              const removeClassFn = removeClass.bind(this, element,
...
angular.module("ui.bootstrap.dateparser", [])
    ...
    // Pulled from https://github.com/mbostock/d3/blob/master/src/format/requote.js
      ...
      this.getParser = function(key) {
        const f = getFormatCodeToRegex(key);
      ...
      function createParser(format) {
        const map = []; const regex = format.split("");

        // check for literal values
        const quoteIndex = format.indexOf("'");
        if (quoteIndex > -1) {
          let inLiteral = false;
          format = format.split("");
          for (let i = quoteIndex; i < format.length; i++) {
            if (inLiteral) {
              if (format[i] === "'") {
        ...
        angular.forEach(formatCodeToRegex, function(data) {
          const index = format.indexOf(data.key);
          ...
          if (index > -1) {
            ...
            for (let i = index + 1, n = index + data.key.length; i < n; i++) {
              regex[i] = "";
              format[i] = "$";
            }
            format = format.join("");
      ...
      function constructFormatterFromIdx(format, i) {
        const currentPosStr = format.substr(i);
        for (let j = 0; j < formatCodeToRegex.length; j++) {
          if (new RegExp("^" + formatCodeToRegex[j].key).test(currentPosStr)) {
            const data = formatCodeToRegex[j];
            return {
      ...
      this.parse = function(input, format, baseDate) {
        ...
        if (results && results.length) {
          ...
          for (let i = 1, n = results.length; i < n; i++) {
            const mapper = map[i - 1];
            if (mapper.matcher === "Z") {
              tzOffset = true;
            }
          ...
          const datesetter = tzOffset ? Date.prototype.setUTCFullYear :
          Date.prototype.setFullYear;
          const timesetter = tzOffset ? Date.prototype.setUTCHours :
...
angular.module("ui.bootstrap.isClass", [])
    .directive("uibIsClass", [
      ...
      function($animate) {
      //                    11111111          22222222
        const ON_REGEXP = /^\s*([\s\S]+?)\s+on\s+([\s\S]+?)\s*$/;
        //                    11111111           22222222
        const IS_REGEXP = /^\s*([\s\S]+?)\s+for\s+([\s\S]+?)\s*$/;

        const dataPerTracked = {};
        ...
        return {
          ...
          compile: function(tElement, tAttrs) {
            ...
            const instances = [];
            let expToData = {};
            const lastActivated = null;
            const onExpMatches = tAttrs.uibIsClass.match(ON_REGEXP);
            const onExp = onExpMatches[2];
            const expsStr = onExpMatches[1];
            const exps = expsStr.split(",");
            ...
            function addForExp(exp, scope) {
              const matches = exp.match(IS_REGEXP);
              const clazz = scope.$eval(matches[1]);
              const compareWithExp = matches[2];
              let data = expToData[exp];
              if (!data) {
                const watchFn = function(compareWithVal) {
                  let newActivated = null;
                  instances.some(function(instance) {
                    const thisVal = instance.scope.$eval(onExp);
            ...
            function removeScope(e) {
              const removedScope = e.targetScope;
              const index = linkedScopes.indexOf(removedScope);
              linkedScopes.splice(index, 1);
              instances.splice(index, 1);
              if (linkedScopes.length) {
                const newWatchScope = linkedScopes[0];
...
angular.module("ui.bootstrap.datepicker", ["ui.bootstrap.dateparser", "ui.bootstrap.isClass"])
    ...
    .controller("UibDatepickerController", ["$scope", "$element", "$attrs", "$parse", "$interpolate", "$locale", "$log", "dateFilter", "uibDatepickerConfig", "$datepickerLiteralWarning", "$datepickerSuppressError", "uibDateParser",
      function($scope, $element, $attrs, $parse, $interpolate, $locale, $log, dateFilter, datepickerConfig, $datepickerLiteralWarning, $datepickerSuppressError, dateParser) {
        ...
        this.init = function(ngModelCtrl_) {
          ...
          const date = ngModelCtrl.$modelValue ? new Date(ngModelCtrl.$modelValue) : new Date();
        ...
        this.render = function() {
          if (ngModelCtrl.$viewValue) {
            const date = new Date(ngModelCtrl.$viewValue);
        ...
        this.refreshView = function() {
          if (this.element) {
            ...
            let date = ngModelCtrl.$viewValue ? new Date(ngModelCtrl.$viewValue) : null;
        ...
        this.createDateObject = function(date, format) {
          let model = ngModelCtrl.$viewValue ? new Date(ngModelCtrl.$viewValue) : null;
          model = dateParser.fromTimezone(model, ngModelOptions.getOption("timezone"));
          let today = new Date();
          today = dateParser.fromTimezone(today, ngModelOptions.getOption("timezone"));
          const time = this.compare(date, today);
          const dt = {
            date: date,
        ...
        $scope.select = function(date) {
          if ($scope.datepickerMode === self.minMode) {
            let dt = ngModelCtrl.$viewValue ? dateParser.fromTimezone(new Date(ngModelCtrl.$viewValue), ngModelOptions.getOption("timezone")) : new Date(0, 0, 0, 0, 0, 0, 0);
        ...
        $scope.move = function(direction) {
          const year = self.activeDate.getFullYear() + direction * (self.step.years || 0);
          const month = self.activeDate.getMonth() + direction * (self.step.months || 0);
        ...
        $scope.keydown = function(evt) {
          const key = $scope.keys[evt.which];
        ...
        function extractOptions(ngModelCtrl) {
          ...
          // ng-model-options defaults timezone to null; don't let its precedence squash a non-null value
            const timezone = ngModelCtrl.$options.getOption("timezone") ||
    ...
    .controller("UibDaypickerController", ["$scope", "$element", "dateFilter", function(scope, $element, dateFilter) {
      ...
      this.getDates = function(startDate, n) {
        const dates = new Array(n); const current = new Date(startDate); let i = 0; let date;
        while (i < n) {
      ...
      this._refreshView = function() {
        ...
        const firstDayOfMonth = new Date(this.activeDate);

        firstDayOfMonth.setFullYear(year, month, 1);

        const difference = this.startingDay - firstDayOfMonth.getDay();
        const numDisplayedFromPreviousMonth = difference > 0 ?
        ...
        if (scope.showWeeks) {
          ...
          const numWeeks = scope.rows.length;
      ...
      this.handleKeyDown = function(key, evt) {
        let date = this.activeDate.getDate();
        ...
        if (key === "left") {
          ...
          const month = this.activeDate.getMonth() + (key === "pageup" ? - 1 : 1);
    ...
    .controller("UibMonthpickerController", ["$scope", "$element", "dateFilter", function(scope, $element, dateFilter) {
      ...
      this.compare = function(date1, date2) {
        const _date1 = new Date(date1.getFullYear(), date1.getMonth());
        const _date2 = new Date(date2.getFullYear(), date2.getMonth());
        _date1.setFullYear(date1.getFullYear());
        _date2.setFullYear(date2.getFullYear());
        return _date1 - _date2;
      };
      ...
      this.handleKeyDown = function(key, evt) {
        let date = this.activeDate.getMonth();
        ...
        if (key === "left") {
          ...
          const year = this.activeDate.getFullYear() + (key === "pageup" ? - 1 : 1);
    ...
    .controller("UibYearpickerController", ["$scope", "$element", "dateFilter", function(scope, $element, dateFilter) {
      ...
      this._refreshView = function() {
        const years = new Array(range); let date;

        for (let i = 0, start = getStartingYear(this.activeDate.getFullYear()); i < range; i++) {
          date = new Date(this.activeDate);
      ...
      this.handleKeyDown = function(key, evt) {
        let date = this.activeDate.getFullYear();
    ...
    .directive("uibDatepicker", function() {
      return {
        ...
        link: function(scope, element, attrs, ctrls) {
          const datepickerCtrl = ctrls[0]; const ngModelCtrl = ctrls[1];
    ...
    .directive("uibDaypicker", function() {
      return {
        ...
        link: function(scope, element, attrs, ctrls) {
          const datepickerCtrl = ctrls[0];
          const daypickerCtrl = ctrls[1];
 ...
 */
     ...
     */
       ...
       *
       * @return {element} The closest positioned ancestor.
       */
       ...
       * return a width of 0.
       *
       * @return {number} The width of the browser scollbar.
       */
       ...
       *   default is false.
       *
       * @return {boolean} Whether the element is scrollable.
       */
...
angular.module("ui.bootstrap.datepickerPopup", ["ui.bootstrap.datepicker", "ui.bootstrap.position"])
    ...
    .controller("UibDatepickerPopupController", ["$scope", "$element", "$attrs", "$compile", "$log", "$parse", "$window", "$document", "$rootScope", "$uibPosition", "dateFilter", "uibDateParser", "uibDatepickerPopupConfig", "$timeout", "uibDatepickerConfig", "$datepickerPopupLiteralWarning",
      function($scope, $element, $attrs, $compile, $log, $parse, $window, $document, $rootScope, $position, dateFilter, dateParser, datepickerPopupConfig, $timeout, datepickerConfig, $datepickerPopupLiteralWarning) {
        const cache = {};
        let isHtml5DateInput = false;
        let dateFormat; let closeOnDateSelection; let appendToBody; let onOpenFocus;
        let datepickerPopupTemplateUrl; let datepickerTemplateUrl; let popupEl; let datepickerEl; let scrollParentEl;
        let ngModel; let ngModelOptions; let $popup; let altInputFormats; const watchListeners = [];
        ...
        this.init = function(_ngModel_) {
          ...
          if (datepickerPopupConfig.html5Types[$attrs.type]) {
            ...
            $attrs.$observe("uibDatepickerPopup", function(value, oldValue) {
              const newDateFormat = value || datepickerPopupConfig.datepickerPopup;
        ...
        $scope.dateSelection = function(dt) {
          $scope.date = dt;
          const date = $scope.date ? dateParser.filter($scope.date, dateFormat) : null; // Setting to NULL is necessary for form validators to function
          $element.val(date);
          ...
          if (value) {
            if (!$scope.disabled) {
              $timeout(function() {
                ...
                const placement = $attrs.popupPlacement ? $attrs.popupPlacement : datepickerPopupConfig.placement;
        ...
        function parseDateString(viewValue) {
          let date = dateParser.parse(viewValue, dateFormat, $scope.date);
          if (isNaN(date)) {
            for (let i = 0; i < altInputFormats.length; i++) {
        ...
        function parseDate(viewValue) {
          ...
          if (angular.isString(viewValue)) {
            const date = parseDateString(viewValue);
            if (!isNaN(date)) {
        ...
        function documentClickBind(event) {
          ...
          const popup = $popup[0];
          const dpContainsTarget = $element[0].contains(event.target);
          // The popup node may not be an element node
          // In some browsers (IE) only element nodes have the 'contains' function
          const popupContainsTarget = popup.contains !== undefined && popup.contains(event.target);
        ...
        function positionPopup() {
          if ($scope.isOpen) {
            const dpElement = angular.element($popup[0].querySelector(".uib-datepicker-popup"));
            const placement = $attrs.popupPlacement ? $attrs.popupPlacement : datepickerPopupConfig.placement;
            const position = $position.positionElements($element, dpElement, placement, appendToBody);
...
angular.module("ui.bootstrap.dropdown", ["ui.bootstrap.multiMap", "ui.bootstrap.position"])
    ...
    .service("uibDropdownService", ["$document", "$rootScope", "$$multiMap", function($document, $rootScope, $$multiMap) {
      let openScope = null;
      const openedContainers = $$multiMap.createNew();

      this.isOnlyOpen = function(dropdownScope, appendTo) {
        const openedDropdowns = openedContainers.get(appendTo);
        if (openedDropdowns) {
          const openDropdown = openedDropdowns.reduce(function(toClose, dropdown) {
      ...
      this.open = function(dropdownScope, element, appendTo) {
        ...
        const openedDropdowns = openedContainers.get(appendTo);
        if (openedDropdowns) {
          const openedScopes = openedDropdowns.map(function(dropdown) {
      ...
      this.close = function(dropdownScope, element, appendTo) {
        ...
        const openedDropdowns = openedContainers.get(appendTo);
        if (openedDropdowns) {
          const dropdownToClose = openedDropdowns.reduce(function(toClose, dropdown) {
            if (dropdown.scope === dropdownScope) {
              return dropdown;
            }
      ...
      // unbound this event handler. So check openScope before proceeding.
        ...
        const dropdownElement = openScope.getDropdownElement();
      ...
      this.keybindFilter = function(evt) {
        ...
        const dropdownElement = openScope.getDropdownElement();
        const toggleElement = openScope.getToggleElement();
        const dropdownElementTargeted = dropdownElement && dropdownElement[0].contains(evt.target);
        const toggleElementTargeted = toggleElement && toggleElement[0].contains(evt.target);
        if (evt.which === 27) {
          evt.stopPropagation();
          openScope.focusToggleElement();
          closeDropdown();
    ...
    .controller("UibDropdownController", ["$scope", "$element", "$attrs", "$parse", "uibDropdownConfig", "uibDropdownService", "$animate", "$uibPosition", "$document", "$compile", "$templateRequest", function($scope, $element, $attrs, $parse, dropdownConfig, uibDropdownService, $animate, $position, $document, $compile, $templateRequest) {
      ...
      let templateScope;
      const appendToOpenClass = dropdownConfig.appendToOpenClass;
      ...
      scope.focusDropdownEntry = function(keyCode) {
        const elems = self.dropdownMenu ? // If append to body is used.
      ...
      scope.$watch("isOpen", function(isOpen, wasOpen) {
        ...
        if (angular.isDefined($attrs.dropdownAppendTo)) {
          const appendToEl = $parse($attrs.dropdownAppendTo)(scope);
          if (appendToEl) {
        ...
        if (angular.isDefined($attrs.dropdownAppendToBody)) {
          const appendToBodyValue = $parse($attrs.dropdownAppendToBody)(scope);
        ...
        if (appendTo && self.dropdownMenu) {
          const pos = $position.positionElements($element, self.dropdownMenu, "bottom-left", true);
          let css;
          let rightalign;
          let scrollbarPadding;
          ...
          if (!appendToBody) {
            const appendOffset = $position.offset(appendTo);
        ...
        const openContainer = appendTo ? appendTo : $element;
        const dropdownOpenClass = appendTo ? appendToOpenClass : openClass;
        const hasOpenClass = openContainer.hasClass(dropdownOpenClass);
        const isOnlyOpen = uibDropdownService.isOnlyOpen($scope, appendTo);
        ...
        if (isOpen) {
          ...
          if (self.dropdownMenuTemplateUrl) {
            ...
            const newEl = angular.element("<ul class=\"dropdown-menu\"></ul>");
            self.dropdownMenu.replaceWith(newEl);
            self.dropdownMenu = newEl;
          }
    ...
    .directive("uibDropdownMenu", function() {
      return {
        ...
        link: function(scope, element, attrs, dropdownCtrl) {
          ...
          const tplUrl = attrs.templateUrl;
          if (tplUrl) {
 ...
 */
    .provider("$uibResolve", function() {
      ...
      this.$get = ["$injector", "$q", function($injector, $q) {
        const resolver = resolve.resolver ? $injector.get(resolve.resolver) : null;
        return {
   ...
   */
    ...
    .factory("$uibModalStack", ["$animate", "$animateCss", "$document",
      ...
      function($animate, $animateCss, $document, $compile, $rootScope, $q, $$multiMap, $$stackedMap, $uibPosition) {
        ...
        let backdropDomEl; let backdropScope;
        const openedWindows = $$stackedMap.createNew();
        const openedClasses = $$multiMap.createNew();
        const $modalStack = {
          NOW_CLOSING_EVENT: "modal.stack.now-closing",
        };
        let topModalIndex = 0;
        let previousTopOpenedModal = null;
        const ARIA_HIDDEN_ATTRIBUTE_NAME = "data-bootstrap-modal-aria-hidden-count";
        ...
        $get: ["$rootScope", "$q", "$document", "$templateRequest", "$controller", "$uibResolve", "$uibModalStack",
          function($rootScope, $q, $document, $templateRequest, $controller, $uibResolve, $modalStack) {
            ...
            $modal.open = function(modalOptions) {
              ...
              const modalClosedDeferred = $q.defer();
              const modalRenderDeferred = $q.defer();
              ...
              samePromise = promiseChain = $q.all([promiseChain])
                  ...
                  .then(function resolveSuccess(tplAndVars) {
                    const providedScope = modalOptions.scope || $rootScope;

                    const modalScope = providedScope.$new();
                    ...
                    const component = {};
                    let ctrlInstance; let ctrlInstantiate; const ctrlLocals = {};
      ...
      return {
        create: function(ctrl, $scope, $attrs) {
          ...
          ctrl.calculateTotalPages = function() {
            const totalPages = ctrl.itemsPerPage < 1 ? 1 : Math.ceil($scope.totalItems / ctrl.itemsPerPage);
          ...
          $scope.selectPage = function(page, evt) {
            ...
            const clickAllowed = !$scope.ngDisabled || !evt;
    ...
    .directive("uibPager", ["uibPagerConfig", function(uibPagerConfig) {
      return {
        ...
        link: function(scope, element, attrs, ctrls) {
          ...
          const paginationCtrl = ctrls[0]; const ngModelCtrl = ctrls[1];
...
angular.module("ui.bootstrap.pagination", ["ui.bootstrap.paging", "ui.bootstrap.tabindex"])
    .controller("UibPaginationController", ["$scope", "$attrs", "$parse", "uibPaging", "uibPaginationConfig", function($scope, $attrs, $parse, uibPaging, uibPaginationConfig) {
      ...
      let maxSize = angular.isDefined($attrs.maxSize) ? $scope.$parent.$eval($attrs.maxSize) : uibPaginationConfig.maxSize;
      const rotate = angular.isDefined($attrs.rotate) ? $scope.$parent.$eval($attrs.rotate) : uibPaginationConfig.rotate;
      const forceEllipses = angular.isDefined($attrs.forceEllipses) ? $scope.$parent.$eval($attrs.forceEllipses) : uibPaginationConfig.forceEllipses;
      const boundaryLinkNumbers = angular.isDefined($attrs.boundaryLinkNumbers) ? $scope.$parent.$eval($attrs.boundaryLinkNumbers) : uibPaginationConfig.boundaryLinkNumbers;
      const pageLabel = angular.isDefined($attrs.pageLabel) ? function(idx) {
            ...
            compile: function(tElem, tAttrs) {
                ...
                // check if tooltip exists and is not empty
                  ...
                  if (!positionTimeout) {
                    positionTimeout = $timeout(function() {
                      ...
                      const initialHeight = angular.isDefined(tooltip.offsetHeight) ? tooltip.offsetHeight : tooltip.prop("offsetHeight");
                      ...
                      adjustmentTimeout = $timeout(function() {
                        const currentHeight = angular.isDefined(tooltip.offsetHeight) ? tooltip.offsetHeight : tooltip.prop("offsetHeight");
               ...
               */
                ...
                function prepTriggers() {
                  const showTriggers = []; const hideTriggers = [];
...
angular.module("ui.bootstrap.progressbar", [])
    ...
    .controller("UibProgressController", ["$scope", "$attrs", "uibProgressConfig", function($scope, $attrs, progressConfig) {
      const self = this;
      const animate = angular.isDefined($attrs.animate) ? $scope.$parent.$eval($attrs.animate) : progressConfig.animate;
      ...
      this.addBar = function(bar, element, attrs) {
        ...
        bar.recalculatePercentage = function() {
          const totalPercentage = self.bars.reduce(function(total, bar) {
...
angular.module("ui.bootstrap.rating", [])
    ...
    .controller("UibRatingController", ["$scope", "$attrs", "uibRatingConfig", function($scope, $attrs, ratingConfig) {
      ...
      this.init = function(ngModelCtrl_) {
        ...
        const tmpTitles = angular.isDefined($attrs.titles) ? $scope.$parent.$eval($attrs.titles) : ratingConfig.titles;
    ...
    .directive("uibRating", function() {
      return {
        ...
        link: function(scope, element, attrs, ctrls) {
          const ratingCtrl = ctrls[0]; const ngModelCtrl = ctrls[1];
...
angular.module("ui.bootstrap.tabs", [])
    ...
    .controller("UibTabsetController", ["$scope", function($scope) {
      ...
      ctrl.select = function(index, evt) {
        if (!destroyed) {
          const previousIndex = findTabIndex(oldIndex);
          const previousSelected = ctrl.tabs[previousIndex];
          if (previousSelected) {
          ...
          const selected = ctrl.tabs[index];
          if (selected) {
    ...
    .directive("uibTabContentTransclude", function() {
      return {
        ...
        link: function(scope, elm, attrs) {
          const tab = scope.$eval(attrs.uibTabContentTransclude).tab;
...
angular.module("ui.bootstrap.timepicker", [])
    ...
    .controller("UibTimepickerController", ["$scope", "$element", "$attrs", "$parse", "$log", "$locale", "uibTimepickerConfig", function($scope, $element, $attrs, $parse, $log, $locale, timepickerConfig) {
      let hoursModelCtrl; let minutesModelCtrl; let secondsModelCtrl;
      let selected = new Date();
      const watchers = [];
      let ngModelCtrl = {$setViewValue: angular.noop}; // nullModelCtrl
      const meridians = angular.isDefined($attrs.meridians) ? $scope.$parent.$eval($attrs.meridians) : timepickerConfig.meridians || $locale.DATETIME_FORMATS.AMPMS;
      const padHours = angular.isDefined($attrs.padHours) ? $scope.$parent.$eval($attrs.padHours) : true;
      ...
      this.init = function(ngModelCtrl_, inputs) {
        ...
        const minutesInputEl = inputs.eq(1);
        const secondsInputEl = inputs.eq(2);

        hoursModelCtrl = hoursInputEl.controller("ngModel");
        minutesModelCtrl = minutesInputEl.controller("ngModel");
        secondsModelCtrl = secondsInputEl.controller("ngModel");

        const mousewheel = angular.isDefined($attrs.mousewheel) ? $scope.$parent.$eval($attrs.mousewheel) : timepickerConfig.mousewheel;

        if (mousewheel) {
          this.setupMousewheelEvents(hoursInputEl, minutesInputEl, secondsInputEl);
        }

        const arrowkeys = angular.isDefined($attrs.arrowkeys) ? $scope.$parent.$eval($attrs.arrowkeys) : timepickerConfig.arrowkeys;
        if (arrowkeys) {
      ...
      $scope.noIncrementHours = function() {
        const incrementedSelected = addMinutes(selected, hourStep * 60);
        return disabled || incrementedSelected > max ||
        incrementedSelected < selected && incrementedSelected < min;
      };

      $scope.noDecrementHours = function() {
        const decrementedSelected = addMinutes(selected, -hourStep * 60);
        return disabled || decrementedSelected < min ||
        decrementedSelected > selected && decrementedSelected > max;
      };

      $scope.noIncrementMinutes = function() {
        const incrementedSelected = addMinutes(selected, minuteStep);
        return disabled || incrementedSelected > max ||
        incrementedSelected < selected && incrementedSelected < min;
      };

      $scope.noDecrementMinutes = function() {
        const decrementedSelected = addMinutes(selected, -minuteStep);
        return disabled || decrementedSelected < min ||
        decrementedSelected > selected && decrementedSelected > max;
      };

      $scope.noIncrementSeconds = function() {
        const incrementedSelected = addSeconds(selected, secondStep);
        return disabled || incrementedSelected > max ||
        incrementedSelected < selected && incrementedSelected < min;
      };

      $scope.noDecrementSeconds = function() {
        const decrementedSelected = addSeconds(selected, -secondStep);
        return disabled || decrementedSelected < min ||
        decrementedSelected > selected && decrementedSelected > max;
      };
      ...
      if ($attrs.showMeridian) {
        watchers.push($scope.$parent.$watch($parse($attrs.showMeridian), function(value) {
          ...
          // Evaluate from template
            const hours = getHoursFromTemplate(); const minutes = getMinutesFromTemplate();
      ...
      function getHoursFromTemplate() {
        ...
        const valid = $scope.showMeridian ? hours > 0 && hours < 13 :
        hours >= 0 && hours < 24;
      ...
      function getMinutesFromTemplate() {
        ...
        const valid = minutes >= 0 && minutes < 60;
      ...
      this.setupInputEvents = function(hoursInputEl, minutesInputEl, secondsInputEl) {
        ...
        const invalidate = function(invalidHours, invalidMinutes, invalidSeconds) {
      ...
      function addSeconds(date, seconds) {
        const dt = new Date(date.getTime() + seconds * 1000);
    ...
    .directive("uibTimepicker", ["uibTimepickerConfig", function(uibTimepickerConfig) {
      return {
        ...
        link: function(scope, element, attrs, ctrls) {
          const timepickerCtrl = ctrls[0]; const ngModelCtrl = ctrls[1];
 ...
 */
    ...
    .controller("UibTypeaheadController", ["$scope", "$element", "$attrs", "$compile", "$parse", "$q", "$timeout", "$document", "$window", "$rootScope", "$$debounce", "$uibPosition", "uibTypeaheadParser",
      function(originalScope, element, attrs, $compile, $parse, $q, $timeout, $document, $window, $rootScope, $$debounce, $position, typeaheadParser) {
        ...
        let modelCtrl; let ngModelOptions;
        ...
        const isSelectEvent = attrs.typeaheadShouldSelect ? $parse(attrs.typeaheadShouldSelect) : function(scope, vals) {
        ...
        const appendTo = attrs.typeaheadAppendTo ?
        originalScope.$eval(attrs.typeaheadAppendTo) : null;

        const focusFirst = originalScope.$eval(attrs.typeaheadFocusFirst) !== false;

        // If input matches an item of the list exactly, select it automatically
        const selectOnExact = attrs.typeaheadSelectOnExact ? originalScope.$eval(attrs.typeaheadSelectOnExact) : false;
        ...
        let inputsContainer; let hintInputElem;
        ...
        this.init = function(_modelCtrl) {
          ...
          modelCtrl.$formatters.push(function(modelValue) {
            let candidateViewValue; let emptyViewValue;
            const locals = {};
    ...
    .directive("uibTypeaheadMatch", ["$templateRequest", "$compile", "$parse", function($templateRequest, $compile, $parse) {
      return {
        ...
        link: function(scope, element, attrs) {
          const tplUrl = $parse(attrs.templateUrl)(scope.$parent) || "uib/template/typeahead/typeahead-match.html";
...
angular.module("uib/template/rating/rating.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("uib/template/rating/rating.html",
    ...
    "    <i ng-repeat-end ng-mouseenter=\"enter($index + 1)\" ng-click=\"rate($index + 1)\" class=\"fa-star\" ng-class=\"$index < value && (r.stateOn || 'fas') || (r.stateOff || 'far')\" ng-attr-title=\"{{r.title}}\"></i>\n" +
    "</span>\n" +
    "");
}]);
...
angular.module("uib/template/timepicker/timepicker.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("uib/template/timepicker/timepicker.html",
    ...
    "      <td class=\"uib-increment hours\"><a ng-click=\"incrementHours()\" ng-class=\"{disabled: noIncrementHours()}\" class=\"btn btn-link\" ng-disabled=\"noIncrementHours()\" tabindex=\"-1\"><span class=\"fas fa-chevron-up\"></span></a></td>\n" +
    "      <td>&nbsp;</td>\n" +
    "      <td class=\"uib-increment minutes\"><a ng-click=\"incrementMinutes()\" ng-class=\"{disabled: noIncrementMinutes()}\" class=\"btn btn-link\" ng-disabled=\"noIncrementMinutes()\" tabindex=\"-1\"><span class=\"fas fa-chevron-up\"></span></a></td>\n" +
    "      <td ng-show=\"showSeconds\">&nbsp;</td>\n" +
    "      <td ng-show=\"showSeconds\" class=\"uib-increment seconds\"><a ng-click=\"incrementSeconds()\" ng-class=\"{disabled: noIncrementSeconds()}\" class=\"btn btn-link\" ng-disabled=\"noIncrementSeconds()\" tabindex=\"-1\"><span class=\"fas fa-chevron-up\"></span></a></td>\n" +
    ...
    "    </tr>\n" +
    "    <tr class=\"text-center\" ng-show=\"::showSpinners\">\n" +
    "      <td class=\"uib-decrement hours\"><a ng-click=\"decrementHours()\" ng-class=\"{disabled: noDecrementHours()}\" class=\"btn btn-link\" ng-disabled=\"noDecrementHours()\" tabindex=\"-1\"><span class=\"fas fa-chevron-down\"></span></a></td>\n" +
    "      <td>&nbsp;</td>\n" +
    "      <td class=\"uib-decrement minutes\"><a ng-click=\"decrementMinutes()\" ng-class=\"{disabled: noDecrementMinutes()}\" class=\"btn btn-link\" ng-disabled=\"noDecrementMinutes()\" tabindex=\"-1\"><span class=\"fas fa-chevron-down\"></span></a></td>\n" +
    "      <td ng-show=\"showSeconds\">&nbsp;</td>\n" +
    "      <td ng-show=\"showSeconds\" class=\"uib-decrement seconds\"><a ng-click=\"decrementSeconds()\" ng-class=\"{disabled: noDecrementSeconds()}\" class=\"btn btn-link\" ng-disabled=\"noDecrementSeconds()\" tabindex=\"-1\"><span class=\"fas fa-chevron-down\"></span></a></td>\n" +
    "      <td ng-show=\"showMeridian\"></td>\n" +
    "    </tr>\n" +
    "  </tbody>\n" +
    "</table>\n" +
...
dialogModule.factory("$dialog", ["$rootScope", "$uibModal", "$sce", function($rootScope, $uibModal, $sce) {
  function dialog(modalOptions, resultFn) {
    const dialog = $uibModal.open(modalOptions);
  ...
  return {
    ...
    messageBox: function(title, message, buttons, resultFn) {
      const scope = angular.extend($rootScope.$new(false), {title: title, message: $sce.trustAsHtml(message), buttons: buttons});
      return dialog(modalOptions("template/messageBox/message.html", "MessageBoxController", scope), function(result) {
        const value = resultFn ? resultFn(result) : undefined;
        scope.$destroy();
        return value;
      });
    },
...
dialogModule.controller("MessageBoxController", ["$scope", "$uibModalInstance", function($scope, $uibModalInstance) {
