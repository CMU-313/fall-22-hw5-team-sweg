...
(function (factory) {
  ...
  const widgetsSortable = $.widget("ui.sortable", $.ui.mouse, {
    ...
    _intersectsWith: function (item) {
      ...
      const isOverElementHeight =
        this.options.axis === "x" || (y1 + dyClick > t && y1 + dyClick < b);
      const isOverElementWidth =
    ...
    _intersectsWithSides: function (item) {
      const isOverBottomHalf = this._isOverAxis(
        this.positionAbs.top + this.offset.click.top,
        item.top + item.height / 2,
        item.height
      );
      const isOverRightHalf = this._isOverAxis(
        this.positionAbs.left + this.offset.click.left,
