...
(function(f) {
  ...
  let define; let module; let exports; return (function e(t, n, r) {
    function s(o, u) {
      if (!n[o]) {
        if (!t[o]) {
          const a=typeof require=="function"&&require; if (!u&&a) return a(o, !0); if (i) return i(o, !0); const f=new Error("Cannot find module '"+o+"'"); throw f.code="MODULE_NOT_FOUND", f;
        } const l=n[o]={exports: {}}; t[o][0].call(l.exports, function(e) {
          const n=t[o][1][e]; return s(n?n:e);
        }, l, l.exports, e, t, n, r);
      } return n[o].exports;
    } var i=typeof require=="function"&&require; for (let o=0; o<r.length; o++)s(r[o]); return s;
  })({1: [function(require, module, exports) {
    const addDataAttr = require("./utils").addDataAttr;
   ...
   */
    ...
    const less = module.exports = require("./index")(window, options);

    window.less = less;

    let css; let head; let style;
    ...
    module.exports = function(window, less, options) {
      function errorHTML(e, rootHref) {
        ...
        const elem = window.document.createElement("div"); let timer; let content; const errors = [];
        ...
        const errorline = function(e, i, classname) {
  ...
  /* global window, XMLHttpRequest */
    ...
    module.exports = function() {
      const functionRegistry = require("./../less/functions/function-registry");
    ...
    module.exports = function(window, options) {
      ...
      const FileManager = require("./file-manager")(options, less.logger);
      const fileManager = new FileManager();
      environment.addFileManager(fileManager);
      less.FileManager = FileManager;

      require("./log-listener")(less, options);
      const errors = require("./error-reporting")(window, less, options);
      const cache = less.cache = options.cache || require("./cache")(window, options, less.logger);
      ...
      less.unwatch = function() {
        clearInterval(less.watchTimer); this.watchMode = false; return false;
      };
      ...
      less.refresh = function(reload, modifyVars, clearFileCache) {
        ...
        return new Promise(function(resolve, reject) {
          let startTime; let endTime; let totalMilliseconds; let remainingSheets;
 ...
 */
  ...
  // todo - do the same for the toCSS ?
    ...
    const abstractFileManager = function() {
    };
    ...
    // diff between two paths to create a relative path
      ...
      let i; let max; let urlDirectories; let baseUrlDirectories; let diff = "";
    ...
    // urlParts[5] = parameters
      ...
      const returner = {}; let directories = []; let i; let baseUrlParts;
    ...
    const environment = function(externalEnvironment, fileManagers) {
    ...
    function colorBlend(mode, color1, color2) {
      const ab = color1.alpha; let cb; // backdrop
      const as = color2.alpha; let cs; // source
      let ar; let cr; const r = []; // result
    ...
    colorFunctions = {
      ...
      rgba: function(r, g, b, a) {
        const rgb = [r, g, b].map(function(c) {
          return scaled(c, 255);
        });
        a = number(a);
    ...
    module.exports = function(environment) {
      ...
      const functionRegistry = require("./function-registry");
      const fallback = function(functionThis, node) {
      ...
      functionRegistry.add("data-uri", function(mimetypeNode, filePathNode) {
        ...
        const fileManager = environment.getFileManager(filePath, currentDirectory, this.context, environment, true);
        ...
        const fileSync = fileManager.loadFileSync(filePath, currentDirectory, this.context, environment);
    ...
    const defaultFunc = {
      eval: function() {
        const v = this.value_; const e = this.error_;
        if (e) {
          throw e;
        }
        if (v != null) {
    ...
    const functionCaller = function(name, context, index, currentFileInfo) {
          ...
          return true;
            ...
            .map(function(item) {
              if (item.type === "Expression") {
                const subNodes = item.value.filter(function(item) {
                  if (item.type === "Comment") {
                    return false;
                  }
                  return true;
    ...
    const minMax = function(isMin, args) {
      ...
      let i; let j; let current; let currentUnified; let referenceUnified; let unit; let unitStatic; let unitClone;
      const order = []; // elems only contains original argument values.
      const values = {}; // key is the unit.toString() for unified Dimension values,
    ...
    functionRegistry.addMultiple({
      ...
      percentage: function(n) {
        const result = mathHelper._math(function(num) {
          return num * 100;
        }, "%", n);
    ...
    module.exports = function(environment) {
      ...
      functionRegistry.add("svg-gradient", function(direction) {
        ...
        let i; let color; let position; let positionValue; let alpha;
    ...
    const isa = function(n, Type) {
      return (n instanceof Type) ? Keyword.True : Keyword.False;
    };
    const isunit = function(n, unit) {
      if (unit === undefined) {
    ...
    const getItemsFromNode = function(node) {
    ...
    //  'reference' - whether the file should not be output and only output parts that are referenced
      ...
      const ImportManager = function(context, rootFileInfo) {
     ...
     */
      ImportManager.prototype.push = function(path, tryAppendLessExtension, currentFileInfo, importOptions, callback) {
        ...
        const promise = fileManager.loadFile(path, currentFileInfo.currentDirectory, this.context, environment,
    ...
    module.exports = function(environment, fileManagers) {
      let SourceMapOutput; let SourceMapBuilder; let ParseTree; let ImportManager; let Environment;
    ...
    const LessError = module.exports = function LessError(e, importManager, currentFilename) {
      Error.call(this);
    ...
    module.exports = function(SourceMapBuilder) {
      const ParseTree = function(root, imports) {
        this.root = root;
        this.imports = imports;
      };

      ParseTree.prototype.toCSS = function(options) {
        let evaldRoot; const result = {}; let sourceMapBuilder;
        try {
    ...
    module.exports = function(input, fail) {
      const len = input.length; let level = 0; let parenLevel = 0;
      let lastOpening; let lastOpeningParen; let lastMultiComment; let lastMultiCommentEndBrace;
      const chunks = []; let emitFrom = 0;
      let chunkerCurrentIndex; let currentChunkStartIndex; let cc; let cc2; let matched;
    ...
    module.exports = function() {
      ...
      let furthest; // furthest index the parser has gone to
      let furthestPossibleErrorMessage; let // if this is furthest we got to, this is the probably cause
        chunks; // chunkified input
      let current; // current chunk
      let currentPos; // index of current chunk, in `input`
      const parserInput = {};
    ...
    const Parser = function Parser(context, imports, fileInfo) {
      ...
      return {
        ...
        parse: function(str, callback, additionalData) {
          let root; let error = null; let globalVars; let modifyVars; let ignored; let preText = "";
        ...
        //
          "primary": function() {
            const mixin = this.mixin; let root = []; let node;
          ...
          //
            quoted: function() {
              let str; const index = parserInput.i; let isEscaped = false;
            ...
            //
            call: function() {
              let name; let nameLC; let args; let alpha; const index = parserInput.i;
            ...
            arguments: function() {
              const argsSemiColon = []; const argsComma = [];
              let expressions = [];
              let isSemiColonSeparated; let value; let arg;
            ...
            //
            url: function() {
              let value; const index = parserInput.i;
            ...
            //
            variable: function() {
              let name; const index = parserInput.i;
            ...
            variableCurly: function() {
              let curly; const index = parserInput.i;
            ...
            //
            javascript: function() {
              let js; const index = parserInput.i;
          ...
          "extend": function(isRule) {
            let elements; let e; const index = parserInput.i; let option; let extendList; let extend;
          ...
          //
            call: function() {
              const s = parserInput.currentChar(); let important = false; const index = parserInput.i; let elemIndex;
              let elements; let elem; let e; let c; let args;
            ...
            args: function(isCall) {
              ...
              let expressions = []; const argsSemiColon = []; const argsComma = [];
              let isSemiColonSeparated; let expressionContainsNamed; let name; let nameLoop;
              let value; let arg; let expand;
            ...
            //
            definition: function() {
              let name; let params = []; let match; let ruleset; let cond; let variadic = false;
          ...
          //
          "element": function() {
            let e; let c; let v; const index = parserInput.i;
          ...
          "selector": function(isLess) {
            const index = parserInput.i; let elements; let extendList; let c; let e; let allExtends; let when; let condition;
          ...
          //
          "ruleset": function() {
            let selectors; let s; let rules; let debugInfo;
          ...
          "rule": function(tryAnonymous) {
            let name; let value; const startOfRule = parserInput.i; const c = parserInput.currentChar(); let important; let merge; let isVariable;
            ...
            if (name) {
              ...
              // where each item is a tree.Keyword or tree.Variable
                ...
                const tryValueFirst = !tryAnonymous && (context.compress || isVariable);
          ...
          //
          "import": function() {
            let path; let features; const index = parserInput.i;
          ...
          "importOptions": function() {
            let o; const options = {}; let optionName; let value;
          ...
          "mediaFeature": function() {
            const entities = this.entities; const nodes = []; let e; let p;
            parserInput.save();
            do {
          ...
          "mediaFeatures": function() {
            const entities = this.entities; const features = []; let e;
            do {
              e = this.mediaFeature();
              if (e) {
                features.push(e);
          ...
          "media": function() {
            let features; let rules; let media; let debugInfo; const index = parserInput.i;
          ...
          //
          "directive": function() {
            const index = parserInput.i; let name; let value; let rules; let nonVendorSpecificName;
            let hasIdentifier; let hasExpression; let hasUnknown; let hasBlock = true; let isRooted = true;
          ...
          "multiplication": function() {
            let m; let a; let op; let operation; let isSpaced;
            m = this.operand();
            if (m) {
              ...
              while (true) {
                ...
                if (!op) {
                  parserInput.forget(); break;
                }

                a = this.operand();

                if (!a) {
                  parserInput.restore(); break;
                }
                parserInput.forget();
          ...
          "conditions": function() {
            let a; let b; const index = parserInput.i; let condition;
          ...
          "condition": function() {
            let result; let logical; let next;
            function or() {
          ...
          "atomicCondition": function() {
            const entities = this.entities; const index = parserInput.i; let a; let b; let c; let op;
          ...
          //
          "operand": function() {
            const entities = this.entities; let negate;
          ...
          //
          "expression": function() {
            const entities = []; let e; let delim;
          ...
          "ruleProperty": function() {
            let name = []; const index = []; let s; let k;
   ...
   * Plugin Manager
   */
    const PluginManager = function(less) {
      this.less = less;
      this.visitors = [];
      this.preProcessors = [];
      this.postProcessors = [];
   ...
   */
    ...
    const FunctionImporter = module.exports = function FunctionImporter(context, fileInfo) {
    ...
    module.exports = function(SourceMapOutput, environment) {
      const SourceMapBuilder = function(options) {
    ...
    module.exports = function(environment) {
      const SourceMapOutput = function(options) {
        this._css = [];
    ...
    Call.prototype.eval = function(context) {
      const args = this.args.map(function(a) {
        return a.eval(context);
      });
      let result; const funcCaller = new FunctionCaller(this.name, context, this.index, this.currentFileInfo);
    ...
    Color.prototype.toCSS = function(context, doNotCompress) {
      const compress = context && context.compress && !doNotCompress; let color; let alpha;
    ...
    Color.prototype.toHSL = function() {
      ...
      let h; let s; const l = (max + min) / 2; const d = max - min;
    ...
    Color.fromKeyword = function(keyword) {
      let c; const key = keyword.toLowerCase();
    ...
    Dimension.prototype.convertTo = function(conversions) {
      let value = this.value; const unit = this.unit.clone();
      let i; let groupName; let group; let targetUnit; let derivedConversions = {}; let applyUnit;
    ...
    const Directive = function(name, value, rules, index, currentFileInfo, debugInfo, isRooted, visibilityInfo) {
      let i;
    ...
    Directive.prototype.accept = function(visitor) {
      const value = this.value; const rules = this.rules;
      if (rules) {
    ...
    Directive.prototype.eval = function(context) {
      let mediaPathBackup; let mediaBlocksBackup; let value = this.value; let rules = this.rules;
    ...
    Directive.prototype.outputRuleset = function(context, output, rules) {
      const ruleCnt = rules.length; let i;
      ...
      // Non-compressed
      const tabSetStr = "\n" + Array(context.tabLevel).join("  "); const tabRuleStr = tabSetStr + "  ";
      if (!ruleCnt) {
    ...
    const Element = function(combinator, value, index, currentFileInfo, info) {
    ...
    Element.prototype.toCSS = function(context) {
      context = context || {};
      let value = this.value; const firstSelector = context.firstSelector;
    ...
    const Extend = function Extend(selector, option, index, currentFileInfo, visibilityInfo) {
    ...
    Import.prototype.doEval = function(context) {
      ...
      if (this.options.inline) {
        ...
        const newImport = new Import(this.evalPath(context), features, this.options, this.index);
    ...
    const Media = function(value, features, index, currentFileInfo, visibilityInfo) {
      ...
      const selectors = (new Selector([], null, null, this.index, this.currentFileInfo)).createEmptySelectors();
    ...
    Media.prototype.eval = function(context) {
      ...
      const media = new Media(null, [], this.index, this.currentFileInfo, this.visibilityInfo());
      if (this.debugInfo) {
    ...
    MixinCall.prototype.eval = function(context) {
      let mixins; let mixin; let mixinPath; const args = []; let arg; let argValue;
      const rules = []; let match = false; let i; let m; let f; let isRecursive; let isOneFound;
      const candidates = []; let candidate; const conditionResult = []; let defaultResult; const defFalseEitherCase = -1;
      const defNone = 0; const defTrue = 1; const defFalse = 2; let count; let originalRuleset; let noArgumentsFilter;

      function calcDefGroup(mixin, mixinPath) {
        let f; let p; let namespace;
    ...
    /* jshint boss:true */
      ...
      let i; let j; let val; let name; let isNamedFound; let argIndex; let argsLength = 0;
    ...
    Definition.prototype.matchArgs = function(args, context) {
      const allArgsCnt = (args && args.length) || 0; let len; const optionalParameters = this.optionalParameters;
    ...
    Quoted.prototype.eval = function(context) {
      const that = this; let value = this.value;
      const javascriptReplacement = function(_, exp) {
        return String(that.evaluateJavaScript(exp, context));
      };
      const interpolationReplacement = function(_, name) {
        const v = new Variable("@" + name, that.index, that.currentFileInfo).eval(context, true);
    ...
    const Rule = function(name, value, important, merge, index, currentFileInfo, inline, variable) {
      this.name = name;
    ...
    function evalName(context, name) {
      let value = ""; let i; const n = name.length;
      const output = {add: function(s) {
        value += s;
      }};
      for (i = 0; i < n; i++) {
    ...
    Rule.prototype.eval = function(context) {
      let strictMathBypass = false; let name = this.name; let evaldValue; let variable = this.variable;
    ...
    const RulesetCall = function(variable) {
    ...
    Ruleset.prototype.eval = function(context) {
      const thisSelectors = this.selectors; let selectors;
      let selCnt; let selector; let i; let hasOnePassingSelector = false;
      ...
      const rsRules = ruleset.rules; let rsRuleCnt = rsRules ? rsRules.length : 0;
    ...
    Ruleset.prototype.evalImports = function(context) {
      const rules = this.rules; let i; let importRules;
      if (!rules) {
        return;
      }
    ...
    Ruleset.prototype.rulesets = function() {
      ...
      const filtRules = []; const rules = this.rules; const cnt = rules.length;
      let i; let rule;
    ...
    Ruleset.prototype.find = function(selector, self, filter) {
      self = self || this;
      const rules = []; let match; let foundMixins;
    ...
    Ruleset.prototype.genCSS = function(context, output) {
      ...
      let ruleNodes = [];
      let debugInfo; // Line number debugging
      let rule;
      let path;
      ...
      if (!this.root) {
        ...
        const paths = this.paths; const pathCnt = paths.length;
        let pathSubCnt;
    ...
    Ruleset.prototype.joinSelector = function(paths, context, selector) {
      ...
      function addReplacementIntoPath(beginningPath, addPath, replacedElement, originalSelector) {
        let newSelectorPath; let lastSelector; let newJoinedSelector;
        ...
        // this also allows + a { & .b { .a & { ... though not sure why you would want to do that
          let combinator = replacedElement.combinator; const parentEl = addPath[0].elements[0];
      ...
      // == [[.a] [.c]] [[.b] [.c]]
      //
        let i; let j; let k; let currentElements; let newSelectors; let selectorsMultiplied; let sel; let el; let hadParentSelector = false; let length; let lastSelector;
        ...
        for (i = 0; i < inSelector.elements.length; i++) {
          ...
          if (el.value !== "&") {
            ...
            // on to the current list of selectors to add
              ...
              const nestedPaths = []; var replaced; const replacedNewSelectors = [];
      ...
      let i; let newPaths; let hadParentSelector;
    ...
    Selector.prototype.createDerived = function(elements, extendList, evaldCondition) {
      ...
      const newSelector = new Selector(elements, extendList || this.extendList, null, this.index, this.currentFileInfo, info);
    ...
    Selector.prototype.createEmptySelectors = function() {
      ...
      const sels = [new Selector([el], null, null, this.index, this.currentFileInfo)];
    ...
    Selector.prototype.eval = function(context) {
      ...
      let elements = this.elements; let extendList = this.extendList;
    ...
    Unit.prototype.toString = function() {
      let i; let returnStr = this.numerator.join("*");
    ...
    Unit.prototype.usedUnits = function() {
      let group; const result = {}; let mapUnit; let groupName;
    ...
    Unit.prototype.cancel = function() {
      const counter = {}; let atomicUnit; let i;
    ...
    Variable.prototype.eval = function(context) {
      let variable; let name = this.name;
    ...
    const ExtendFinderVisitor = function() {
    ...
    ExtendFinderVisitor.prototype = {
      ...
      visitRuleset: function(rulesetNode, visitArgs) {
        ...
        let i; let j; let extend; const allSelectorsExtendList = []; let extendList;

        // get &:extend(.a); rules which apply to all selectors in this ruleset
        const rules = rulesetNode.rules; const ruleCnt = rules ? rules.length : 0;
        for (i = 0; i < ruleCnt; i++) {
    ...
    const ProcessExtendsVisitor = function() {
      this._visitor = new Visitor(this);
    };
    ...
    ProcessExtendsVisitor.prototype = {
      ...
      // processed if we look at each selector at a time, as is done in visitRuleset
        ...
        let extendIndex; let targetExtendIndex; let matches; const extendsToAdd = []; let newSelector; const extendVisitor = this; let selectorPath;
        let extend; let targetExtend; let newExtend;
      ...
      visitRuleset: function(rulesetNode, visitArgs) {
        ...
        let matches; let pathIndex; let extendIndex; const allExtends = this.allExtendsStack[this.allExtendsStack.length - 1];
        const selectorsToAdd = []; const extendVisitor = this; let selectorPath;
      ...
      //
        let haystackSelectorIndex; let hackstackSelector; let hackstackElementIndex; let haystackElement;
        let targetCombinator; let i;
        const extendVisitor = this;
        const needleElements = extend.selector.elements;
        const potentialMatches = []; let potentialMatch; const matches = [];
      ...
      visitDirective: function(directiveNode, visitArgs) {
        let newAllExtends = directiveNode.allExtends.concat(this.allExtendsStack[this.allExtendsStack.length - 1]);
    ...
    ImportVisitor.prototype = {
      ...
      visitImport: function(importNode, visitArgs) {
        ...
        if (!importNode.css || inlineCSS) {
          const context = new contexts.Eval(this.context, this.context.frames.slice(0));
    ...
    const JoinSelectorVisitor = function() {
      this.contexts = [[]];
      this._visitor = new Visitor(this);
    };
    ...
    const SetTreeVisibilityVisitor = function(visible) {
    ...
    const CSSVisitorUtils = function(context) {
      this._visitor = new Visitor(this);
    ...
    ToCSSVisitor.prototype = {
        ...
        // be considered illegal css as it has to be on the first line
          if (this.charset) {
            if (directiveNode.debugInfo) {
              const comment = new tree.Comment("/* " + directiveNode.toCSS(this._context).replace(/\n/g, "") + " */\n");
      ...
      // at this point rulesets are nested into each other
        ...
        // remove invisible paths
          ...
          const nodeRules = rulesetNode.rules; let nodeRuleCnt = nodeRules ? nodeRules.length : 0;
    ...
    const Visitor = function(implementation) {
    ...
    Visitor.prototype = {
      ...
      flatten: function(arr, out) {
        ...
        let cnt; let i; let item;
        let nestedCnt; let j; let nestedItem;
   ...
   */
    ...
    function handleResolved(self, deferred) {
      asap(function() {
        const cb = self._65 === 1 ? deferred.onFulfilled : deferred.onRejected;
        if (cb === null) {
          if (self._65 === 1) {
        ...
        const ret = tryCallOne(cb, self._55);
        if (ret === IS_ERROR) {
    ...
    function finale(self) {
      ...
      if (self._40 === 2) {
        for (let i = 0; i < self._72.length; i++) {
   ...
   */
    function doResolve(fn, promise) {
      let done = false;
      const res = tryCallTwo(fn, function(value) {
        if (done) return;
        done = true;
    ...
    function valuePromise(value) {
      const p = new Promise(Promise._61);
      p._65 = 1;
      p._55 = value;
      return p;
    }
