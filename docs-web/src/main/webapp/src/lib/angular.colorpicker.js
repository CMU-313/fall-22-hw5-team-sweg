      ...
      prepareValues: function (format) {
        ...
        return {
          ...
          transform:
            "to" +
            (thisFormat === "hex"
              ? thisFormat.charAt(0).toUpperCase() + thisFormat.slice(1)
              : thisFormat.length > 3
              ? thisFormat.toUpperCase().slice(0, -1)
              : thisFormat.toUpperCase()),
        };
      },
        ...
        link: function (scope, element, attrs, ngModel) {
          ...
          element.colorpicker().on("changeColor", function (event) {
            element.val(
              element
                .data("colorpicker")
                .format(event.color[thisFormat.transform]())
            );
            scope.$apply(
  ...
  .directive("colorpicker", [
    ...
    function (helper) {
      return {
        ...
        template:
          '<div class="input-append color" data-color="rgb(0, 0, 0)" data-color-format="">' +
