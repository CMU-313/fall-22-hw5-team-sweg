...
(function (global, factory) {
  ...
  const ng =
    ng_from_import && ng_from_import.module ? ng_from_import : ng_from_global;
  ...
  const notImplemented = function (fnname) {
    return function () {
      throw new Error(
        fnname + "(): No coreservices implementation for UI-Router is loaded."
      );
    };
  };
  const services = {
  ...
  function _equals(o1, o2) {
    ...
    if (
      predicates.map(any).reduce(function (b, fn) {
        return b || !!fn(tup);
      }, false)
    ) {
      return false;
  ...
  /**
   * @module common
   */ /** for typedoc */ const Queue = /** @class */ (function () {
   ...
   * @coreapi
   * @module trace
   */ /** for typedoc */ /** @hidden */ function uiViewString(uiview) {
    if (!uiview) {
  ...
  function normalizedCat(input) {
    return isNumber(input)
      ...
      : exports.Category[exports.Category[input]];
  }
  /** @hidden */
  const consoleLog = Function.prototype.bind.call(console.log, console);
  /** @hidden */
  const consoletable = isFunction(console.table)
    ? console.table.bind(console)
    : consoleLog.bind(console);
  /**
  ...
  const Trace = /** @class */ (function () {
    ...
    Trace.prototype.traceHookInvocation = function (step, trans, options) {
      ...
      const context =
        ...
        "unknown";
      const name = functionToString(step.registeredHook.callback);
      console.log(
        transLbl(trans) +
          ":   Hook -> " +
          event +
    ...
    Trace.prototype.traceHookResult = function (
      ...
      console.log(
        transLbl(trans) +
          ":   <- Hook returned: " +
          maxLength(200, stringify(hookResult))
      );
    };
    /** @internalapi called by ui-router code */
    Trace.prototype.traceResolvePath = function (path, when, trans) {
      ...
      console.log(
        transLbl(trans) + ":         Resolving " + path + " (" + when + ")"
      );
    };
    /** @internalapi called by ui-router code */
    Trace.prototype.traceResolvableResolved = function (resolvable, trans) {
      ...
      console.log(
        transLbl(trans) +
          ...
          resolvable +
          " to: " +
          maxLength(200, stringify(resolvable.data))
      );
    };
    /** @internalapi called by ui-router code */
    ...
    Trace.prototype.traceViewSync = function (pairs) {
      ...
      const mapping = pairs
        .map(function (_a) {
          const uiViewData = _a[0];
          const config = _a[1];
          const uiView = uiViewData.$type + ":" + uiViewData.fqn;
          const view =
            config &&
            config.viewDecl.$context.name +
              ": " +
              config.viewDecl.$name +
              " (" +
              config.viewDecl.$type +
              ")";
  ...
  const TransitionHook = /** @class */ (function () {
    function TransitionHook(transition, stateContext, registeredHook, options) {
      ...
      this.isSuperseded = function () {
        return (
          _this.type.hookPhase === exports.TransitionHookPhase.RUN &&
          !_this.options.transition.isActive()
        );
      };
    ...
    TransitionHook.prototype.toString = function () {
      ...
      const registeredHook = _a.registeredHook;
      const event = parse("traceData.hookType")(options) || "internal";
      const context =
        parse("traceData.context.state.name")(options) ||
        parse("traceData.context")(options) ||
  ...
  const RegisteredHook = /** @class */ (function () {
    ...
    RegisteredHook.prototype._getMatchingNodes = function (treeChanges) {
      ...
      return paths.reduce(function (mn, pathtype) {
        ...
        const isStateHook =
          pathtype.scope === exports.TransitionHookScope.STATE;
        const path = treeChanges[pathtype.name] || [];
        const nodes = isStateHook ? path : [tail(path)];
        mn[pathtype.name] = _this._matchingNodes(
          nodes,
   ...
   */ const HookBuilder = /** @class */ (function () {
    ...
    HookBuilder.prototype.buildHooks = function (hookType) {
      ...
      const makeTransitionHooks = function (hook) {
        ...
        return matchingNodes.map(function (node) {
          ...
          const state =
            hookType.criteriaMatchPath.scope ===
            exports.TransitionHookScope.STATE
              ? node.state.self
              : null;
    ...
    HookBuilder.prototype.getMatchingHooks = function (hookType, treeChanges) {
      const isCreate =
        hookType.hookPhase === exports.TransitionHookPhase.CREATE;
      ...
      return registries
        ...
        .filter(
          assertPredicate(isArray, "broken event named: " + hookType.name)
        ) // Sanity check
   ...
   * @coreapi
   * @module params
   */ /** for typedoc */ /** @hidden */ const hasOwn =
  ...
  function getType(cfg, urlType, location, id, paramTypes) {
    ...
    if (!cfg.type) {
      const type =
        location === exports.DefType.CONFIG
          ? "any"
  ...
  const Param = /** @class */ (function () {
    function Param(id, type, config, location, urlMatcherFactory) {
      ...
      type = arrayMode
        ? type.$asArray(arrayMode, location === exports.DefType.SEARCH)
        : type;
      const isOptional =
        config.value !== undefined || location === exports.DefType.SEARCH;
      ...
      const squash = getSquashPolicy(
        config,
        isOptional,
        urlMatcherFactory.defaultSquashPolicy()
      );
      const replace = getReplace(config, arrayMode, isOptional, squash);
      const inherit$$1 = isDefined(config.inherit)
        ? !!config.inherit
        : !!type.inherit;
   ...
   * @coreapi
   * @module resolve
   */ /** for typedoc */ // TODO: explicitly make this user configurable
  ...
  const Resolvable = /** @class */ (function () {
    ...
    Resolvable.prototype.resolve = function (resolveContext, trans) {
      ...
      const getResolvableDependencies = function () {
        return $q.all(
          resolveContext.getDependencies(_this).map(function (resolvable) {
      ...
      const maybeWaitForRx =
        this.getPolicy(state).async === "RXWAIT" ? waitForRx : identity;
  ...
  const ResolveContext = /** @class */ (function () {
    ...
    ResolveContext.prototype.getTokens = function () {
      return this._path
        .reduce(function (acc, node) {
          return acc.concat(
            node.resolvables.map(function (r) {
              return r.token;
            })
          );
        }, [])
    ...
    ResolveContext.prototype.subContext = function (state) {
      return new ResolveContext(
        PathUtils.subPath(this._path, function (node) {
    ...
    ResolveContext.prototype.getDependencies = function (resolvable) {
      ...
      const subPath =
        PathUtils.subPath(this._path, function (x) {
          return x === node;
        }) || this._path;
  ...
  const Transition = /** @class */ (function () {
    ...
    function Transition(fromPath, targetState, router) {
      ...
      const onCreateHooks = this._hookBuilder.buildHooksForPhase(
        exports.TransitionHookPhase.CREATE
      );
    ...
    Transition.prototype.toString = function () {
      ...
      const to = isObject(toStateOrName) ? toStateOrName.name : toStateOrName;
  ...
  function initDefaultTypes() {
    ...
    extend(ParamTypes.prototype, {
      ...
      int: makeDefaultType({
        ...
        is: function (val$$1) {
          return (
            !isNullOrUndefined(val$$1) &&
            this.decode(val$$1.toString()) === val$$1
          );
        },
        pattern: /-?\d+/,
      }),
      ...
      date: makeDefaultType({
        ...
        equals: function (l, r) {
          return ["getFullYear", "getMonth", "getDate"].reduce(function (
            acc,
            fn
          ) {
  ...
  /** @module path */ /** for typedoc */ /** @module resolve */ /** for typedoc */ /** @module state */ /** for typedoc */ const parseUrl =
    function (url) {
      if (!isString(url)) {
        return false;
      }
  ...
  const getUrlBuilder = function ($urlMatcherFactoryProvider, root$$1) {
    return function urlBuilder(state) {
      ...
      return parsed && parsed.root
        ? url
        : ((parent && parent.navigable) || root$$1()).url.append(url);
    };
  };
  ...
  const StateBuilder = /** @class */ (function () {
    ...
    StateBuilder.prototype.build = function (state) {
      const _a = this;
      const matcher = _a.matcher;
      const builders = _a.builders;
  ...
  /** @module state */ /** for typedoc */ const StateMatcher =
    /** @class */ (function () {
  ...
  /** @module state */ /** for typedoc */ /** @internalapi */ const StateQueueManager =
    /** @class */ (function () {
      ...
      StateQueueManager.prototype.flush = function () {
        const _this = this;
        const _a = this;
        const queue = _a.queue;
        const states = _a.states;
        const builder = _a.builder;
   ...
   * @coreapi
   * @module state
   */ /** for typedoc */ const StateRegistry = /** @class */ (function () {
    /** @internalapi */
    ...
    StateRegistry.prototype._deregisterTree = function (state) {
      ...
      var getChildren = function (states) {
        ...
        return children.length === 0
          ? children
          : children.concat(getChildren(children));
      };
    ...
    StateRegistry.prototype.deregister = function (stateOrName) {
      ...
      this.listeners.forEach(function (listener) {
        return listener(
          "deregistered",
          deregisteredStates.map(function (s) {
            return s.self;
          })
        );
      });
  ...
  const UrlMatcher = /** @class */ (function () {
      ...
      const matchDetails = function (m, isSearch) {
        ...
        const makeRegexpType = function (regexp) {
          return inherit(paramTypes.type(isSearch ? "query" : "path"), {
            pattern: new RegExp(
              regexp,
              _this.config.caseInsensitive ? "i" : undefined
            ),
          });
        };
        return {
      ...
      while ((m = placeholder.exec(pattern$$1))) {
        ...
        this._params.push(
          paramFactory.fromPath(
            p.id,
            p.type,
            this.config.paramMap(p.cfg, false)
          )
        );
          ...
          while ((m = searchPlaceholder.exec(search))) {
            ...
            this._params.push(
              paramFactory.fromSearch(
                p.id,
                p.type,
                this.config.paramMap(p.cfg, true)
              )
            );
            last = placeholder.lastIndex;
            // check if ?&
    ...
    UrlMatcher.prototype.exec = function (path, search, hash, options) {
      ...
      const pathParams = allParams.filter(function (param) {
        return !param.isSearch();
      });
      const searchParams = allParams.filter(function (param) {
        return param.isSearch();
      });
      const nPathSegments = this._cache.path
        .map(function (urlm) {
    ...
    UrlMatcher.prototype.format = function (values$$1) {
      ...
      const pathString = pathSegmentsAndParams.reduce(function (acc, x) {
        ...
        const squash = x.squash;
        const encoded = x.encoded;
        const param = x.param;
      ...
      const queryString = queryParams
        .map(function (paramDetails) {
          const param = paramDetails.param;
          const squash = paramDetails.squash;
          let encoded = paramDetails.encoded;
          const isDefaultValue = paramDetails.isDefaultValue;
    ...
    UrlMatcher.compare = function (a, b) {
      ...
      const weightsB = weights(b);
      padArrays(weightsA, weightsB, 0);
      let cmp;
      let i;
      const pairs$$1 = arrayTuples(weightsA, weightsB);
   ...
   */ const UrlMatcherFactory = /** @class */ (function () {
     ...
     */
    UrlMatcherFactory.prototype.type = function (
      name,
      definition,
      definitionFn
    ) {
   ...
   */ const UrlRuleFactory = /** @class */ (function () {
    ...
    UrlRuleFactory.isUrlRule = function (obj) {
      return (
        obj &&
        ["type", "match", "handler"].every(function (key) {
  ...
  const UrlRouter = /** @class */ (function () {
    ...
    UrlRouter.prototype.listen = function (enabled) {
      ...
      if (enabled === false) {
        ...
        return (this._stopFn =
          this._stopFn ||
          this._router.urlService.onChange(function (evt) {
   ...
   */ const ViewService = /** @class */ (function () {
    ...
    ViewService.prototype.sync = function () {
      ...
      const matchingConfigPair = function (uiView) {
        const matchingConfigs = _this._viewConfigs.filter(
    ...
    ViewService.normalizeUIViewTarget = function (context, rawViewName) {
      ...
      if (relativeMatch.exec(uiViewContextAnchor)) {
        const anchor = uiViewContextAnchor
          .split(".")
    ...
    ViewService.matches = function (uiViewsByFqn, uiView) {
      return function (viewConfig) {
        ...
        return (
          vc.$uiViewContextAnchor === (uiViewContext && uiViewContext.name)
        );
      };
    };
    return ViewService;
   ...
   * @coreapi
   * @module url
   */ /** */ /** @hidden */ const makeStub = function (keys) {
    return keys.reduce(
      function (acc, key) {
  ...
  /** @hidden */ const locationServicesFns = [
    "url",
    "path",
    "search",
    "hash",
  ...
  /** @hidden */ const locationConfigFns = [
    "port",
    "protocol",
    "host",
    "baseHref",
  ...
  /** @module hooks */ /** */ function addCoreResolvables(trans) {
    trans.addResolvable(
      {
        token: UIRouter,
        deps: [],
   ...
   */ const loadEnteringViews = function (transition) {
    ...
    return $q
      .all(
        enteringViews.map(function (view) {
  ...
  var TransitionService = /** @class */ (function () {
    ...
    TransitionService.prototype._defineCorePaths = function () {
      const STATE = exports.TransitionHookScope.STATE;
      const TRANSITION = exports.TransitionHookScope.TRANSITION;
  ...
  var StateService = /** @class */ (function () {
     ...
     * @internalapi
     */
    StateService.prototype._handleInvalidTargetState = function (
      fromPath,
      toState
    ) {
      const _this = this;
      ...
      if (options.reload && !options.reloadState) {
        throw new Error(
          "No such reload state '" +
            (isString(options.reload) ? options.reload : options.reload.name) +
            "'"
        );
      }
      return new TargetState(
      ...
      var rejectedTransitionHandler = function (transition) {
        return function (error) {
          if (error instanceof Rejection) {
            const isLatest =
              router.globals.lastStartedTransitionId === transition.$id;
            ...
            if (
              error.type === exports.RejectType.SUPERSEDED &&
              error.redirected &&
  ...
  var $q = {
    ...
    all: function (promises) {
      ...
      if (isObject(promises)) {
        ...
        const chain = Object.keys(promises).map(function (key) {
  ...
  function parseUrl$1(url) {
    ...
    const _a = splitHash(url).map(orEmptyString);
    const beforehash = _a[0];
    const hash = _a[1];
    const _b = splitQuery(beforehash).map(orEmptyString);
    const path = _b[0];
    const search = _b[1];
  ...
  function locationPluginFactory(
    ...
    return function (router) {
      ...
      const configuration = (router.locationConfig = new configurationClass(
        router,
        isHtml5
      ));
   ...
   * @internalapi
   * @module vanilla
   */ /** */ /** A base `LocationServices` */ const BaseLocationServices =
    /** @class */ (function () {
  ...
  const MemoryLocationService = /** @class */ (function (_super) {
    ...
    MemoryLocationService.prototype._set = function (
      state,
      title,
      url,
      replace
  ...
  const PushStateLocationService = /** @class */ (function (_super) {
    ...
    PushStateLocationService.prototype._get = function () {
      const _a = this._location;
      let pathname = _a.pathname;
      let hash = _a.hash;
      let search = _a.search;
    ...
    PushStateLocationService.prototype._set = function (
      state,
      title,
      url,
      replace
  ...
  const BrowserLocationConfig = /** @class */ (function () {
    ...
    BrowserLocationConfig.prototype.applyDocumentBaseHref = function () {
      ...
      return (this._baseHref = baseTag
        ? baseTag.href.substr(location.origin.length)
        : "");
    };
  ...
  const hashLocationPlugin = locationPluginFactory(
  ...
  const pushStateLocationPlugin = locationPluginFactory(
  ...
  const memoryLocationPlugin = locationPluginFactory(
   ...
   * @preferred
   * @module core
   */ /** for typedoc */ /** @internalapi */ const UIRouterPluginBase =
    /** @class */ (function () {
    ...
    function Ng1ViewConfig(path, viewDecl, factory) {
      ...
      this.getTemplate = function (uiView, context) {
        return _this.component
          ? _this.factory.makeComponentTemplate(
              uiView,
              context,
              _this.component,
  ...
  const TemplateFactory = /** @class */ (function () {
     ...
     * for that string.
     */
    TemplateFactory.prototype.fromProvider = function (
      provider,
      params,
      context
    ) {
   ...
   */ const getStateHookBuilder = function (hookName) {
    return function stateHookBuilder(state, parentFn) {
      ...
      function decoratedNg1Hook(trans, state) {
        ...
        const locals = extend(getLocals(resolveContext), {
          $state$: state,
          $transition$: trans,
        });
  ...
  const mod_state = ng.module("ui.router.state", [
    "ui.router.router",
    "ui.router.util",
    "ui.router.angular1",
  ]);
  const mod_main = ng.module("ui.router", [
    "ui.router.init",
    "ui.router.state",
    "ui.router.angular1",
  ]);
   ...
   * @preferred
   * @module directives
   */ /** for typedoc */ /** @hidden */ function parseStateRef(ref) {
    ...
    function $StateRefDirective($uiRouter, $timeout) {
      ...
      return {
        ...
        link: function (scope, element, attrs, uiSrefActive) {
          ...
          scope.$on(
            "$destroy",
            $uiRouter.stateRegistry.onStatesChanged(update)
          );
          scope.$on(
            "$destroy",
      ...
      var directive = {
        ...
        compile: function (tElement, tAttrs, $transclude) {
          return function (scope, $element, attrs) {
            ...
            const autoScrollExp = attrs["autoscroll"];
            const renderer = getRenderer(attrs, scope);
            let viewConfig = undefined;
            const inherited = $element.inheritedData("$uiView") || rootData;
            const name =
            ...
            const activeUIView = {
              ...
              get creationContext() {
                ...
                const fromParentTag = parse("$uiView.creationContext")(
                  inherited
                );
    ...
    return {
      ...
      compile: function (tElement) {
        ...
        return function (scope, $element) {
          ...
          const cfg = data.$cfg || {
            viewDecl: {},
            getTemplate: ng_from_import.noop,
          };
          ...
          if (isString(cfg.viewDecl.component)) {
            ...
            const getComponentController = function () {
              ...
              return (
                directiveEl &&
                ng.element(directiveEl).data("$" + cmp_1 + "Controller")
              );
            };
            var deregisterWatch_1 = scope.$watch(
  ...
  const hasComponentImpl =
    typeof ng.module("ui.router")["component"] === "function";
  ...
  function registerControllerCallbacks(
    ...
    if (
      ...
      !(cfg.viewDecl.component && hasComponentImpl)
    ) {
    ...
    if (isFunction(controllerInstance.uiOnParamsChanged)) {
      ...
      const paramsUpdated = function ($transition$) {
        ...
        if (
          $transition$ === viewCreationTrans_1 ||
    ...
    if (isFunction(controllerInstance.uiCanExit)) {
      ...
      var prevTruthyAnswer_1 = function (trans) {
        return (
          !!trans &&
          ((trans[cacheProp_1] && trans[cacheProp_1][id_1] === true) ||
            prevTruthyAnswer_1(trans.redirectedFrom()))
        );
      };
      // If a user answered yes, but the transition was later redirected, don't also ask for the new redirect transition
      const wrappedHook = function (trans) {
        let promise;
        const ids = (trans[cacheProp_1] = trans[cacheProp_1] || {});
  ...
  /** @module ng1 */ /** */ /** @hidden */ function $ViewScrollProvider() {
