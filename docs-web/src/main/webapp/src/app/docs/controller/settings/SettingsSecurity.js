...
angular
  ...
  .controller(
    "SettingsSecurity",
    function ($scope, User, $dialog, $uibModal, Restangular, $translate) {
      ...
      $scope.enableTotp = function () {
        ...
        $dialog.messageBox(title, msg, btns, function (result) {
          if (result === "ok") {
            Restangular.one("user/enable_totp")
              .post()
              .then(function (data) {
      ...
      $scope.disableTotp = function () {
        $uibModal
          ...
          .result.then(function (password) {
            ...
            // Disable TOTP
            Restangular.one("user/disable_totp")
              .post("", {
                password: password,
              })
              .then(function () {
      ...
      $scope.testTotp = function (code) {
        Restangular.one("user/test_totp")
          .post("", {
            code: code,
          })
          .then(
