...
angular
  ...
  .controller(
    ...
    function (
      ...
      $scope.displayMode = _.isUndefined(localStorage.fileDisplayMode)
        ? "grid"
        : localStorage.fileDisplayMode;
      ...
      $scope.deleteFile = function (file) {
        const title = $translate.instant(
          "document.view.content.delete_file_title"
        );
        const msg = $translate.instant(
          "document.view.content.delete_file_message"
        );
        const btns = [
          ...
          _.each(files, function (file) {
            const newfile = {
              ...
              status: $translate.instant(
                "document.view.content.upload_pending"
              ),
            };
      ...
      $scope.uploadFile = function (file, newfile, previousFileId) {
        // Upload the file
        newfile.status = $translate.instant(
          "document.view.content.upload_progress"
        );
        return Upload.upload({
          ...
          .error(function (data) {
            newfile.status = $translate.instant(
              "document.view.content.upload_error"
            );
            if (data.type === "QuotaReached") {
              newfile.status +=
                " - " +
                $translate.instant("document.view.content.upload_error_quota");
            }
          });
      };
