...
angular
  ...
  .controller(
    ...
    function (
      ...
      Restangular.one("comment", $stateParams.id)
        .get()
        .then(
          function (data) {
      ...
      $scope.deleteComment = function (comment) {
        ...
        $dialog.messageBox(title, msg, btns, function (result) {
          if (result === "ok") {
            Restangular.one("comment", comment.id)
              .remove()
              .then(function () {
      ...
      $scope.deleteDocument = function (document) {
        ...
        $dialog.messageBox(title, msg, btns, function (result) {
          if (result === "ok") {
            Restangular.one("document", document.id)
              .remove()
              .then(function () {
                $scope.loadDocuments();
      ...
      $scope.showShare = function (share) {
        // Show the link
        const link =
          $location.absUrl().replace($location.path(), "").replace("#", "") +
          "share.html#/share/" +
          $stateParams.id +
          "/" +
          share.id;
        ...
        const msg = $translate.instant(
          "document.view.shared_document_message",
          { link: link }
        );
        const btns = [{ result: "close", label: $translate.instant("close") }];

        if ($rootScope.userInfo.username !== "guest") {
          btns.unshift({
            result: "unshare",
            label: $translate.instant("unshare"),
            cssClass: "btn-danger",
          });
        }
        ...
        $dialog.messageBox(title, msg, btns, function (result) {
          if (result === "unshare") {
            ...
            Restangular.one("share", share.id)
              .remove()
              .then(function () {
                $scope.document.acls = _.reject(
                  $scope.document.acls,
                  function (s) {
        ...
        Restangular.one("route")
          ...
          .then(function (data) {
            ...
            const title = $translate.instant(
              "document.view.workflow_validated_title"
            );
            const msg = $translate.instant(
              "document.view.workflow_validated_message"
            );
            const btns = [
              {
                result: "ok",
