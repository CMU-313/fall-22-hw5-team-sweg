...
angular
  ...
  .controller(
    ...
    function (
      ...
      $scope.offset = 0;
      $scope.currentPage = 1;
      $scope.limit = _.isUndefined(localStorage.documentsPageSize)
        ? "10"
        : localStorage.documentsPageSize;
      $scope.displayMode = _.isUndefined(localStorage.displayMode)
        ? "list"
      ...
      $scope.searchDropdownAnchor = angular.element(
        document.querySelector(".search-dropdown-anchor")
      );
      ...
      $scope.$watch(
        ...
        function () {
          ...
          if (
            $state.current.name === "document.default" ||
            $state.current.name === "document.default.search"
          ) {
            $state.go(
              $scope.search === ""
                ? "document.default"
                : "document.default.search",
              {
                search: $scope.search,
              },
              {
      ...
      $scope.getUserTypeahead = function ($viewValue) {
        ...
        Restangular.one("user/list")
          ...
          .then(function (data) {
            deferred.resolve(
              _.pluck(
                _.filter(data.users, function (user) {
      ...
      $scope.startSearch = function () {
        ...
        if (!_.isEmpty($scope.advsearch.search_simple)) {
          const simplesearch = _.map(
            $scope.advsearch.search_simple.split(/\s+/),
            function (simple) {
        ...
        if (!_.isEmpty($scope.advsearch.search_fulltext)) {
          const fulltext = _.map(
            $scope.advsearch.search_fulltext.split(/\s+/),
            function (full) {
              return "full:" + full;
            }
          );
        ...
        if (!_.isUndefined($scope.advsearch.after_date)) {
          search +=
            "after:" +
            $filter("date")($scope.advsearch.after_date, "yyyy-MM-dd") +
            " ";
        }
        if (!_.isUndefined($scope.advsearch.before_date)) {
          search +=
            "before:" +
            $filter("date")($scope.advsearch.before_date, "yyyy-MM-dd") +
            " ";
        }
        if (!_.isUndefined($scope.advsearch.after_update_date)) {
          search +=
            "uafter:" +
            $filter("date")($scope.advsearch.after_update_date, "yyyy-MM-dd") +
            " ";
        }
        if (!_.isUndefined($scope.advsearch.before_update_date)) {
          search +=
            "ubefore:" +
            $filter("date")($scope.advsearch.before_update_date, "yyyy-MM-dd") +
            " ";
        }
        if (!_.isEmpty($scope.advsearch.tags)) {
          search += _.reduce(
            $scope.advsearch.tags,
            function (s, t) {
              return s + "tag:" + t.name + " ";
            },
            ""
      ...
      $scope.navigationEnabled = _.isUndefined(localStorage.navigationEnabled)
        ? true
        : localStorage.navigationEnabled === "true";

      Restangular.one("tag/list")
        .get()
        .then(function (data) {
      ...
      $scope.navigateUp = function () {
        ...
        $scope.navigateToTag(
          _.findWhere($scope.tags, { id: $scope.navigatedTag.parent })
        );
      };
      ...
      $scope.getTagChildrenShort = function (tag) {
        ...
        return (
          _.pluck(children, "name").join(", ") +
          (tag.children.length > 2 ? "..." : "")
        );
      };
      ...
      $scope.addTagHere = function () {
        $uibModal
          ...
          .result.then(function (tag) {
            ...
            // Create the tag
            tag.parent = $scope.navigatedTag
              ? $scope.navigatedTag.id
              : undefined;
            Restangular.one("tag")
              .put(tag)
              .then(function (data) {
