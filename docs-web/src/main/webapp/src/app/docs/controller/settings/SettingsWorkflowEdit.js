  ...
  .controller(
    ...
    function (
      ...
      $scope.edit = function () {
        ...
        if ($scope.isEdit()) {
          promise = Restangular.one("routemodel", $stateParams.id).post(
            "",
            workflow
          );
        } else {
          promise = Restangular.one("routemodel").put(workflow);
        }
      ...
      $scope.remove = function () {
        const title = $translate.instant(
          "settings.workflow.edit.delete_workflow_title"
        );
        const msg = $translate.instant(
          "settings.workflow.edit.delete_workflow_message"
        );
        const btns = [
        ...
        $dialog.messageBox(title, msg, btns, function (result) {
          if (result === "ok") {
            Restangular.one("routemodel", $stateParams.id)
              .remove()
              .then(
                function () {
                  $scope.loadWorkflows();
