  ...
  .controller(
    ...
    function (
      ...
      if ($stateParams.files) {
        $scope.orphanFiles = _.isArray($stateParams.files)
          ? $stateParams.files
          : [$stateParams.files];
      }
      ...
      $scope.edit = function () {
        ...
        document.relations = _.pluck(
          _.where(document.relations, { source: true }),
          "id"
        );
        ...
        const metadata = _.reject(document.metadata, function (meta) {
          return (
            _.isUndefined(meta.value) || meta.value === "" || meta.value == null
          );
        });
        document.metadata_id = _.pluck(metadata, "id");
        document.metadata_value = _.pluck(metadata, "value");
        document.metadata_value = _.map(
          document.metadata_value,
          function (val) {
        ...
        // Send to server
        if ($scope.isEdit()) {
          promise = Restangular.one("document", $stateParams.id).post(
            "",
            document
          );
        } else {
        ...
        const attachOrphanFiles = function (data) {
          ...
          _.each($scope.orphanFiles, function (fileId) {
            promises.push(
              Restangular.one("file/" + fileId).post("attach", { id: data.id })
            );
          });
          $scope.orphanFiles = [];
              ...
              deferred.promise.then(
                ...
                function (data) {
                  ...
                  $scope.alerts.unshift({
                    ...
                    msg:
                      $translate.instant(
                        "document.edit.document_" +
                          ($scope.isEdit() ? "edited" : "added") +
                          "_with_errors"
                      ) +
                      (data.responseJSON &&
                      data.responseJSON.type === "QuotaReached"
                        ? " - " +
                          $translate.instant("document.edit.quota_reached")
                        : ""),
                  });
      ...
      $scope.loadVocabulary = function (name) {
        Restangular.one("vocabulary", name)
          .get()
