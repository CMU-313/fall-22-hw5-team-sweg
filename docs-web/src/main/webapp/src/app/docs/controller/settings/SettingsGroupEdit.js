  ...
  .controller(
    ...
    function (
      ...
      $scope.edit = function () {
        ...
        if ($scope.isEdit()) {
          promise = Restangular.one("group", $stateParams.name).post("", group);
        } else {
          promise = Restangular.one("group").put(group);
        }
          ...
          function (e) {
            if (e.data.type === "GroupAlreadyExists") {
              var title = $translate.instant(
                "settings.group.edit.edit_group_failed_title"
              );
              var msg = $translate.instant(
                "settings.group.edit.edit_group_failed_message"
              );
              var btns = [
                {
                  result: "ok",
              ...
              var title = $translate.instant(
                "settings.group.edit.group_used_title"
              );
              var msg = $translate.instant(
                "settings.group.edit.group_used_message",
                { name: e.data.message }
              );
              var btns = [
                {
      ...
      $scope.remove = function () {
        const title = $translate.instant(
          "settings.group.edit.delete_group_title"
        );
        const msg = $translate.instant(
          "settings.group.edit.delete_group_message"
        );
        const btns = [
      ...
      $scope.removeMember = function (member) {
        Restangular.one("group/" + $stateParams.name, member)
          .remove()
          .then(function () {
            $scope.group.members.splice(
              $scope.group.members.indexOf(member),
              1
            );
          });
      };
