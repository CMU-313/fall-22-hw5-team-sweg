    ...
    function (
      ...
      const setFile = function (files) {
        ...
        _.each(files, function (value) {
          if (value.id === $stateParams.fileId) {
            $scope.file = value;
            $scope.trustedFileUrl = $sce.trustAsResourceUrl(
              "../api/file/" + $stateParams.fileId + "/data"
            );
          }
        });
      };
      ...
      Restangular.one("file/list")
        ...
        .then(function (data) {
          ...
          if (!$scope.file) {
            Restangular.one("file/" + $stateParams.fileId + "/versions")
              .get()
              .then(function (data) {
                setFile(data.files);
              });
      ...
      $scope.openFileContent = function () {
        window.open(
          "../api/file/" + $stateParams.fileId + "/data?size=content"
        );
      };
