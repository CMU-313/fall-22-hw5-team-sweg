...
angular
  ...
  .controller(
    "TagEdit",
    function ($scope, $stateParams, Restangular, $dialog, $state, $translate) {
      // Retrieve the tag
      Restangular.one("tag", $stateParams.id)
        .get()
        .then(function (data) {
          $scope.tag = data;
      ...
      $scope.edit = function () {
        // Update the server
        Restangular.one("tag", $scope.tag.id)
          .post("", $scope.tag)
          .then(
            function () {},
            function (e) {
              if (e.data.type === "CircularReference") {
                const title = $translate.instant(
                  "tag.edit.circular_reference_title"
                );
                const msg = $translate.instant(
                  "tag.edit.circular_reference_message"
                );
                const btns = [
                  {
                    result: "ok",
